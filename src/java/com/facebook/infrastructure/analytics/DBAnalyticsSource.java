...
/**
 * This class sets up the analytics package to report metrics into Ganglia for
 * the various DB operations such as: reads per second, average read latency,
 * writes per second, average write latency.
 * 
...
public class DBAnalyticsSource implements IAnalyticsSource {
  ...
  /**
   * Create all the required records we intend to display, and register with the
   * AnalyticsContext.
   */
  public DBAnalyticsSource() {
    ...
    AnalyticsContext.instance()
        .setAttribute("units." + METRIC_WRITE_OPS, "w/s");
    /* create the record */
  ...
  public void doUpdates(AnalyticsContext context) {
    ...
    if (readUsageRecord != null) {
      if (readOperations_.get() > 0) {
        ...
        readUsageRecord.setMetric(METRIC_READ_OPS, readOperations_.get()
            / period);
    ...
    if (readLatencyRecord != null) {
      if (readOperations_.get() > 0) {
        ...
        readLatencyRecord.setMetric(METRIC_READ_AVG, readLatencies_.get()
            / readOperations_.get());
    ...
    if (writeUsageRecord != null) {
      if (writeOperations_.get() > 0) {
        ...
        writeUsageRecord.setMetric(METRIC_WRITE_OPS, writeOperations_.get()
            / period);
    ...
    MetricsRecord writeLatencyRecord = context
        .getMetricsRecord(RECORD_WRITE_AVG);
    if (writeLatencyRecord != null) {
      if (writeOperations_.get() > 0) {
        writeLatencyRecord.setTag(TAG_WRITEAVG, TAG_WRITE_AVG);
        writeLatencyRecord.setMetric(METRIC_WRITE_AVG, writeLatencies_.get()
            / writeOperations_.get());
