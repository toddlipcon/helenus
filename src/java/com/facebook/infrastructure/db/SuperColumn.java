...
public final class SuperColumn implements IColumn, Serializable {
  ...
  private EfficientBidiMap columns_ = new EfficientBidiMap(
  ...
  public String name(String key) {
    ...
    if (column instanceof SuperColumn)
      throw new UnsupportedOperationException(
          "A super column cannot hold other super columns.");
    if (column != null)
      return column.name();
    return null;
  }
  ...
  public int size() {
    /*
     * return the size of the individual columns that make up the super column.
     * This is an APPROXIMATION of the size used only from the Memtable.
     */
    return size_.get();
  }
  ...
  public int serializedSize() {
    /*
     * Size of a super-column is = size of a name (UtfPrefix + length of the
     * string) + 1 byte to indicate if the super-column has been deleted + 4
     * bytes for size of the sub-columns + 4 bytes for the number of sub-columns
     ...
     */
    /*
     * We need to keep the way we are calculating the column size in sync with
     * the way we are calculating the size for the column family serializer.
     */
    return IColumn.UtfPrefix_ + FBUtilities.getUTF8Length(name_)
        + DBConstants.boolSize_ + DBConstants.intSize_ + DBConstants.intSize_
        + getSizeOfAllColumns();
  }
  ...
  public long timestamp(String key) {
    ...
    throw new IllegalArgumentException(
        "Timestamp was requested for a column that does not exist.");
  }
  ...
  public byte[] value(String key) {
    ...
    if (column != null)
      return column.value();
    throw new IllegalArgumentException(
        "Value was requested for a column that does not exist.");
  }
  ...
  public boolean putColumn(IColumn column) {
    if (!(column instanceof SuperColumn))
      throw new UnsupportedOperationException(
          "Only Super column objects should be put here");
...
class SuperColumnSerializer implements ICompactSerializer2<IColumn> {
  public void serialize(IColumn column, DataOutputStream dos)
      throws IOException {
   ...
   */
  private SuperColumn defreezeSuperColumn(DataInputStream dis)
      throws IOException {
  ...
  private void fillSuperColumn(IColumn superColumn, DataInputStream dis)
      throws IOException {
    if (dis.available() == 0)
      return;
  ...
  public IColumn deserialize(DataInputStream dis, IFilter filter)
      throws IOException {
    if (dis.available() == 0)
      return null;
   ...
   * superColumn:column.
   */
  public IColumn deserialize(DataInputStream dis, String name, IFilter filter)
      throws IOException {
    if (dis.available() == 0)
      return null;
    ...
    if (!superColumn.isMarkedForDelete()) {
      ...
      if (size > 0) {
        for (int i = 0; i < size; ++i) {
          IColumn subColumn = Column.serializer().deserialize(dis, names[1],
              filter);
          if (subColumn != null) {
            superColumn.addColumn(subColumn.name(), subColumn);
            break;
