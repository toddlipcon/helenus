...
public class MemtableManager {
  ...
  /*
   * Retrieve column family from the list of Memtables that have been submitted
   * for flush but have not yet been flushed. It also filters out unneccesary
   * columns based on the passed in filter.
   */
  void getColumnFamily(String key, String cfName, String cf, IFilter filter,
    ...
    try {
      ...
      if (memtables != null) {
        ...
        for (int i = size - 1; i >= 0; --i) {
          ColumnFamily columnFamily = memtables.get(i).getLocalCopy(key, cf,
              filter);
