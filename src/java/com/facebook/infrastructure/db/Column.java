...
public final class Column implements IColumn, Serializable {
  ...
  public int size() {
    /*
     * Size of a column is = size of a name (UtfPrefix + length of the string) +
     * 1 byte to indicate if the column has been deleted + 8 bytes for timestamp
     * + 4 bytes which basically indicates the size of the byte array + entire
     * byte array.
     */
  ...
  /*
   * Resolve the column by comparing timestamps if a newer vaue is being input
   * take the change else ignore .
   */
  public boolean putColumn(IColumn column) {
    if (!(column instanceof Column))
      throw new UnsupportedOperationException(
          "Only Column objects should be put here");
...
class ColumnSerializer implements ICompactSerializer2<IColumn> {
  ...
  /**
   * We know the name of the column here so just return it. Filter is pretty
   * much useless in this call and is ignored.
   */
  public IColumn deserialize(DataInputStream dis, String columnName,
    ...
    if (name.equals(columnName)) {
      ...
      if (filter instanceof IdentityFilter) {
        /*
         * If this is being called with identity filter since a column name is
         * passed in we know that this is a final call Hence if the column is
         * found set the filter to done so that we do not look for the column in
         * further files
         */
