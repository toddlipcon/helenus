...
public class Row implements Serializable {
  ...
  /*
   * This is used as oldRow.merge(newRow). Basically we take the newRow and
   * merge it into the oldRow.
   */
  void merge(Row row) {
  ...
  /*
   * This function will repair the current row with the input row what that
   * means is that if there are any differences between the 2 rows then this fn
   * will make the current row take the latest changes .
   */
  ...
  /*
   * This function will calculate the difference between 2 rows and return the
   * resultant row. This assumes that the row that is being submitted is a super
   * set of the current row so it only calculates additional difference and does
   * not take care of what needs to be delted from the current row to make it
   * same as the input row.
   */
