...
class CommitLog {
   ...
   */
  private static void writeCommitLogHeader(String commitLogFileName,
  ...
  /*
   * Generates a file name of the format CommitLog-<table>-<timestamp>.log in
   * the directory specified by the Database Descriptor.
   */
  private void setNextFileName() {
    logFile_ = DatabaseDescriptor.getLogFileLocation()
        + System.getProperty("file.separator") + "CommitLog-" + table_ + "-"
        + System.currentTimeMillis() + ".log";
  }

  /*
   * param @ table - name of table for which we are maintaining this commit log.
   * param @ recoverymode - is commit log being instantiated in in recovery
   * mode.
   */
  ...
  /*
   * This ctor is currently used only for debugging. We are now using it to
   * modify the header so that recovery can be tested in as many scenarios as we
   * could imagine.
   * 
  ...
  /*
   * This is invoked on startup via the ctor. It basically writes a header with
  ...
  private void writeCommitLogHeader(byte[] bytes, boolean reset)
      throws IOException {
  ...
  void recover(List<File> clogs) throws IOException {
    ...
    /*
     * Identify files that we need for processing. This can be done using the
     * information in the header of each file. Simply and the byte[] (which are
     * the headers) and stop at the file where the result is a zero.
     */
  ...
  private void doRecovery(Stack<File> filesNeeded, byte[] header)
      throws IOException {
    Table table = Table.open(table_);
    ...
    while (!filesNeeded.isEmpty()) {
      ...
      // IFileReader reader =
      // SequenceFile.bufferedReader(file.getAbsolutePath(),
      // DatabaseDescriptor.getLogFileSizeThreshold());
      ...
      try {
        ...
        CommitLogHeader clHeader = CommitLogHeader.serializer().deserialize(
            bufIn);
        /* seek to the lowest position */
        int lowPos = CommitLogHeader.getLowestPosition(clHeader);
        /*
         * If lowPos == 0 then we need to skip the processing of this file.
         */
        if (lowPos == 0)
          break;
        else
        ...
        while (!reader.isEOF()) {
          ...
          try {
            ...
            for (String cName : cNames) {
              ...
              if (clHeader.get(id) == 0
                  || reader.getCurrentPosition() < clHeader.getPosition(id))
                row.removeColumnFamily(columnFamily);
            }
            if (!row.isEmpty()) {
              table.applyNow(row);
  ...
  /*
   * Update the header of the commit log if a new column family is encountered
   * for the first time.
   */
  private void updateHeader(Row row) throws IOException {
    ...
    for (String cName : cNames) {
      ColumnFamily columnFamily = columnFamilies.get(cName);
      int id = table.getColumnFamilyId(columnFamily.name());
      if (clHeader_.get(id) == 0
          || (clHeader_.get(id) == 1 && clHeader_.getPosition(id) == 0)) {
        if (clHeader_.get(id) == 0
            || (clHeader_.get(id) == 1 && clHeader_.getPosition(id) == 0)) {
  ...
  /*
   * Adds the specified row to the commit log. This method will reset the file
   * offset to what it is before the start of the operation in case of any
   * problems. This way we can assume that the subsequent commit log entry will
   * override the garbage left over by the previous write.
   */
  ...
  /*
   * This is called on Memtable flush to add to the commit log a token
   * indicating that this column family has been flushed. The bit flag
   * associated with this column family is set in the header and this is used to
   * decide if the log file can be deleted.
   */
  synchronized void onMemtableFlush(String cf,
  ...
  /*
   * Check if old commit logs can be deleted. However we cannot do this anymore
   * in the Fast Sync mode and hence I think we should get rid of Fast Sync mode
   * altogether. If there is a pathological event where few CF's are rarely
   * being updated then their Memtable never gets flushed. This will prevent
   * commit logs from being deleted. WE NEED to fix this using some hueristic
   * and force flushing such Memtables.
   * 
   * param @ cLogCtx The commitLog context . param @ id id of the columnFamily
   * being flushed to disk.
   */
  private void discard(CommitLog.CommitLogContext cLogCtx, int id)
      throws IOException {
    ...
    /*
     * We do any processing only if there is a change in the position in the
     * context. This can happen if an older Memtable's flush comes in after a
     * newer Memtable's flush. Right now this cannot happen since Memtables are
     * flushed on a single thread.
     */
    ...
    /*
     * Loop through all the commit log files in the history. Now process all
     * files that are older than the one in the context. For each of these files
     * the header needs to modified by performing a bitwise & of the header with
     * the header of the file in the context. If we encounter the file in the
     * context in our list of old commit log files then we update the header and
     * write it back to the commit log.
     */
    for (String oldFile : oldFiles) {
      if (oldFile.equals(cLogCtx.file())) {
        /*
         * We need to turn on again. This is because we always keep the bit
         * turned on and the position indicates from where the commit log needs
         * to be read. When a flush occurs we turn off perform & operation and
         * then turn on with the new position.
         */
  ...
  private void checkThresholdAndRollLog(long fileSize) {
    try {
      if (fileSize >= DatabaseDescriptor.getLogFileSizeThreshold()) {
        if (logWriter_.getFileSize() >= DatabaseDescriptor
          ...
          /*
           * We need to zero out positions because the positions in the old file
           * do not make sense in the new one.
           */
          clHeader_.zeroPositions();
          writeCommitLogHeader(clHeader_.toByteArray(), false);
          // Get the list of files in commit log directory if it is greater than
          // a certain number
  ...
  public static void main(String[] args) throws Throwable {
    ...
    for (File file : files) {
      ...
      CommitLogHeader clHeader = CommitLogHeader.serializer()
          .deserialize(bufIn);
      /*
