...
public class RowMutationVerbHandler implements IVerbHandler {
  ...
  public void doVerb(Message message) {
    ...
    logger_.info("ROW MUTATION STAGE: "
        + StageManager.getStageTaskCount(StorageService.mutationStage_));
    /* END DEBUG */
    ...
    try {
      ...
      if (hintedBytes != null && hintedBytes.length > 0) {
        ...
        try {
          RowMutation hintedMutation = new RowMutation(rm.table(),
              HintedHandOffManager.key_);
      ...
      /*
       * WriteResponseMessage writeResponseMessage = new
       * WriteResponseMessage(rm.table(), rm.key(), true); Message response =
       * message.getReply( StorageService.getLocalStorageEndPoint(), new
       * Object[]{writeResponseMessage} );
       * logger_.debug("Sending teh response to " + message.getFrom() +
       * " for key :" + rm.key());
