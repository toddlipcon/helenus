...
public class RowMutation implements Serializable {
  ...
  protected RowMutation(String table, String key,
      Map<String, ColumnFamily> modifications,
      Map<String, ColumnFamily> deletions) {
    table_ = table;
    key_ = key;
  ...
  /*
   * Specify a column family name and the corresponding column family object.
   * param @ cf - column family name param @ columnFamily - the column family.
   */
  ...
  /*
   * Specify a column name and a corresponding value for the column. Column name
   * is specified as <column family>:column. This will result in a ColumnFamily
   * associated with <column family> as name and a Column with <column> as name.
   * 
   * param @ cf - column name as <column family>:<column> param @ value - value
   * associated with the column
   */
  public void add(String cf, byte[] value) throws IOException,
      ColumnFamilyNotDefinedException {
    add(cf, value, 0);
  }
   ...
   * is specified as <column family>:column. This will result in a ColumnFamily
   * associated with <column family> as name and a Column with <column> as name.
   * The columan can be further broken up as super column name : columnname in
   * case of super columns
   * 
   * param @ cf - column name as <column family>:<column> param @ value - value
   * associated with the column param @ timestamp - ts associated with this
   * data.
   */
  public void add(String cf, byte[] value, long timestamp) {
    ...
    if (values.length == 0 || values.length == 1 || values.length > 3)
      throw new IllegalArgumentException("Column Family " + cf
          + " in invalid format. Must be in <column family>:<column> format.");

    ColumnFamily columnFamily = modifications_.get(values[0]);
    if (values.length == 2) {
      if (columnFamily == null) {
        columnFamily = new ColumnFamily(values[0], ColumnFamily
            .getColumnType("Standard"));
      }
  ...
  /*
   * Specify a column name to be deleted. Column name is specified as <column
   * family>:column. This will result in a ColumnFamily associated with <column
   * family> as name and perhaps Column with <column> as name being marked as
   * deleted. TODO : Delete is NOT correct as we do not know the CF type so we
   * need to fix that. param @ cf - column name as <column family>:<column>
   */
  ...
  /*
   * This is equivalent to calling commit. Applies the changes to to the table
   * that is obtained by calling Table.open().
   */
  public void apply() throws IOException, ColumnFamilyNotDefinedException {
    ...
    for (String cfName : cfNames) {
      if (!table.isValidColumnFamily(cfName))
        throw new ColumnFamilyNotDefinedException("Column Family " + cfName
...
class RowMutationSerializer implements ICompactSerializer<RowMutation> {
  private void freezeTheMaps(Map<String, ColumnFamily> map, DataOutputStream dos)
      throws IOException {
    int size = map.size();
    dos.writeInt(size);
    if (size > 0) {
  ...
  public void serialize(RowMutation rm, DataOutputStream dos)
      throws IOException {
