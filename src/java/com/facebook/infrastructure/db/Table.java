...
public class Table {
  /*
   * This class represents the metadata of this Table. The metadata is basically
   * the column family name and the ID associated with this column family. We
   * use this ID in the Commit Log header to determine when a log file that has
   * been rolled can be deleted.
   */
  public static class TableMetadata {
    /* Name of the column family */
    public final static String cfName_ = "TableMetadata";
    /*
     * Name of one of the columns. The other columns are the individual column
    ...
    private static String getFileName() {
      ...
      return DatabaseDescriptor.getMetadataDirectory()
          + System.getProperty("file.separator") + table + "-Metadata.db";
    }
  ...
  static class TableMetadataSerializer implements
      ICompactSerializer<TableMetadata> {
    public void serialize(TableMetadata tmetadata, DataOutputStream dos)
        throws IOException {
  ...
  /**
   * This is the callback handler that is invoked when we have completely been
   * bootstrapped for a single file by a remote host.
   */
  public static class BootstrapCompletionHandler implements IStreamComplete {
    public void onStreamCompletion(String host,
        StreamContextManager.StreamContext streamContext,
        StreamContextManager.StreamStatus streamStatus) throws IOException {
      /*
       * Parse the stream context and the file to the list of SSTables in the
       * associated Column Family Store.
       */
      if (streamContext.getTargetFile().indexOf("-Data.db") != -1) {
        ...
        if (streamContext.getCardinality() != null) {
          ...
          logger_.debug("Done merging "
              + streamContext.getCardinality().count()
              + " keys in the sampler ...");
        }
        String[] peices = FBUtilities.strip(fileName, "-");
        Table.open(peices[0]).getColumnFamilyStore(peices[1]).addToList(
            streamContext.getTargetFile());
      }

      EndPoint to = new EndPoint(host, DatabaseDescriptor.getStoragePort());
      logger_.debug("Sending a bootstrap terminate message with "
          + streamStatus + " to " + to);
      /*
       * Send a StreamStatusMessage object which may require the source node to
       * re-stream certain files.
       */
      StreamContextManager.StreamStatusMessage streamStatusMessage = new StreamContextManager.StreamStatusMessage(
          streamStatus);
      Message message = StreamContextManager.StreamStatusMessage
          .makeStreamStatusMessage(streamStatusMessage);
  ...
  public static class BootStrapInitiateVerbHandler implements IVerbHandler {
    /*
     * Here we handle the BootstrapInitiateMessage. Here we get the array of
     * StreamContexts. We get file names for the column families associated with
     * the files and replace them with the file names as obtained from the
     * column family store on the receiving end.
     */
    public void doVerb(Message message) {
      ...
      try {
        BootstrapInitiateMessage biMsg = BootstrapInitiateMessage.serializer()
            .deserialize(bufIn);
        StreamContextManager.StreamContext[] streamContexts = biMsg
            .getStreamContext();

        Map<String, String> fileNames = getNewNames(streamContexts);
        /*
         * For each of stream context's in the incoming message generate the new
         * file names and store the new file names in the StreamContextManager.
         */
        for (StreamContextManager.StreamContext streamContext : streamContexts) {
          StreamContextManager.StreamStatus streamStatus = new StreamContextManager.StreamStatus(
              streamContext.getTargetFile(), streamContext.getExpectedBytes());
          ...
          String file = new String(DatabaseDescriptor.getDataFileLocation()
              + System.getProperty("file.separator") + newFileName + "-Data.db");
          logger_.debug("Received Data from  : " + message.getFrom() + " "
              + streamContext.getTargetFile() + " " + file);
          streamContext.setTargetFile(file);
          addStreamContext(message.getFrom().getHost(), streamContext,
              streamStatus);
        }

        StreamContextManager.registerStreamCompletionHandler(message.getFrom()
            .getHost(), new Table.BootstrapCompletionHandler());
        ...
        Message doneMessage = new Message(StorageService
    ...
    private Map<String, String> getNewNames(
        StreamContextManager.StreamContext[] streamContexts) {
      /*
       * Mapping for each file with unique CF-i ---> new file name. For eg. for
       * a file with name <Table>-<CF>-<i>-Data.db there is a corresponding
       * <Table>-<CF>-<i>-Index.db. We maintain a mapping from <CF>-<i> to a
       * newly generated file name.
       */
      Map<String, String> fileNames = new HashMap<String, String>();
      /*
       * Get the distinct entries from StreamContexts i.e have one entry per
       * Data/Index file combination
       */
      ...
      Map<String, ColumnFamilyStore> columnFamilyStores = table
    ...
    private boolean isStreamContextForThisColumnFamily(
        StreamContextManager.StreamContext streamContext, String cf) {
      String[] peices = FBUtilities.strip(streamContext.getTargetFile(), "-");
    ...
    private void addStreamContext(String host,
        StreamContextManager.StreamContext streamContext,
  ...
  /*
   * The AnalyticsSource instance which keeps track of statistics reported to
   * Ganglia.
   */
  ...
  public static Table open(String table) {
    ...
    /*
     * Read the config and figure the column families for this table. Set the
     * isConfigured flag so that we do not read config all the time.
     */
  ...
  /*
   * This method is called to obtain statistics about the table. It will return
   * statistics about all the column families that make up this table.
   */
  ...
  /*
   * This method is used to ensure that all keys prior to the specified key, as
   * dtermined by the SSTable index bucket it falls in, are in buffer cache.
   */
  ...
  /*
   * This method is invoked only during a bootstrap process. We basically do a
   * complete compaction since we can figure out based on the ranges whether the
   * files need to be split.
   */
  public BloomFilter.CountingBloomFilter forceCompaction(List<Range> ranges,
    ...
    for (String columnFamily : columnFamilies) {
      ...
      if (cfStore != null) {
        ...
        BloomFilter.CountingBloomFilter cbf2 = cfStore.forceCompaction(ranges,
            target, 0, fileList);
        if (cbf2 == null) {
  ...
  /*
   * This method is an ADMIN operation to force compaction of all SSTables on
   * disk.
   */
  ...
  private Table(String table) {
    ...
    try {
      ...
      Set<String> columnFamilies = tableMetadata_.getColumnFamilies();
      for (String columnFamily : columnFamilies) {
        columnFamilyStores_.put(columnFamily, new ColumnFamilyStore(table,
            columnFamily));
      }
    } catch (IOException ex) {
      logger_.info(LogUtil.throwableToString(ex));
  ...
  Row get(String key) throws IOException {
    ...
    for (String columnFamily : columnFamilies) {
      ColumnFamilyStore cfStore = columnFamilyStores_.get(columnFamily);
      if (cfStore != null) {
        ColumnFamily cf = cfStore.getColumnFamily(key, columnFamily,
            new IdentityFilter());
        if (cf != null)
   ...
   */
  public ColumnFamily get(String key, String cf)
    ...
    if (cfStore != null) {
      ...
      throw new ColumnFamilyNotDefinedException("Column family " + cf
   ...
   */
  public Row getRow(String key, String cf)
      throws ColumnFamilyNotDefinedException, IOException {
    Row row = new Row(key);
   ...
   * Selects only the specified column family for the specified key.
   */
  public Row getRow(String key, String cf, int start, int count)
      throws ColumnFamilyNotDefinedException, IOException {
    Row row = new Row(key);
    String[] values = RowMutation.getColumnAndColumnFamily(cf);
    ColumnFamilyStore cfStore = columnFamilyStores_.get(values[0]);
  ...
  public Row getRow(String key, String cf, long sinceTimeStamp)
      throws ColumnFamilyNotDefinedException, IOException {
    Row row = new Row(key);
    String[] values = RowMutation.getColumnAndColumnFamily(cf);
    ColumnFamilyStore cfStore = columnFamilyStores_.get(values[0]);
   ...
   * 
   * param @ key - key for which data is requested. param @ cf - column family
   * we are interested in. param @ columns - columns that are part of the above
   * column family.
   */
  public Row getRow(String key, String cf, List<String> columns)
      throws IOException {
    Row row = new Row(key);
    String[] values = RowMutation.getColumnAndColumnFamily(cf);
    ColumnFamilyStore cfStore = columnFamilyStores_.get(values[0]);
    ...
    if (cfStore != null) {
      ColumnFamily columnFamily = cfStore.getColumnFamily(key, cf,
          new NamesFilter(new ArrayList(columns)));
      if (columnFamily != null)
        row.addColumnFamily(columnFamily);
    }
    return row;
  ...
  /*
   * This method adds the row to the Commit Log associated with this table. Once
   * this happens the data associated with the individual column families is
   * also written to the column family store's memtable.
   */
      ...
      if (cName.equals(Table.recycleBin_)) {
        ...
        for (IColumn column : columns) {
          ...
          if (column.timestamp() == 1) {
            ...
            cfStore.forceCompaction(null, null, BasicUtilities
                .byteArrayToLong(column.value()), null);
