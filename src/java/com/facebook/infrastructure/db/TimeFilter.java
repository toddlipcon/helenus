...
/**
 * This class provides a filter for fitering out columns that are older than a
 * specific time.
 * 
 * @author pmalik
 * 
...
class TimeFilter implements IFilter {
  ...
  public ColumnFamily filter(String cf, ColumnFamily columnFamily) {
    ...
    if (values.length == 1
      ...
      if (i < columns.size()) {
        ...
        && DatabaseDescriptor.getColumnType(cfName).equals("Super")) {
      /*
       * TODO : For super columns we need to re-visit this issue. For now this
       * fn will set done to true if we are done with atleast one super column
       */
      Collection<IColumn> columns = columnFamily.getAllColumns();
      for (IColumn column : columns) {
  ...
  public IColumn filter(IColumn column, DataInputStream dis) throws IOException {
    long timeStamp = 0;
    /*
     * If its a column instance we need the timestamp to verify if it should be
     * filtered , but at this instance the timestamp is not read so we read the
     * timestamp and set the buffer back so that the rest of desrialization
