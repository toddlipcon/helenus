...
public class SystemTable {
  ...
  public static SystemTable openSystemTable(String tableName)
      throws IOException {
  ...
  public Row get(String key) throws IOException {
    ...
    if (bufOut.getLength() > 0) {
      ...
      /*
       * This buffer contains key and value so we need to strip certain parts
       */
      // read the key
      bufIn.readUTF();
  ...
  /*
   * This is a one time thing and hence we do not need any commit log related
   * activity. Just write in an atomic fashion to the underlying SequenceFile.
   */
  ...
  public void updateToken(BigInteger token) throws IOException {
    if (systemRow_ != null) {
      ...
      long oldTokenColumnTimestamp = columnFamily.getColumn(SystemTable.token_)
          .timestamp();
      /* create the "Token" whose value is the new token. */
      IColumn tokenColumn = new Column(SystemTable.token_, token.toByteArray(),
          oldTokenColumnTimestamp + 1);
      /* replace the old "Token" column with this new one. */
      logger_.debug("Replacing old token "
          + new BigInteger(columnFamily.getColumn(SystemTable.token_).value())
              .toString() + " with token " + token.toString());
  ...
  public static void main(String[] args) throws Throwable {
    ...
    SystemTable.openSystemTable(SystemTable.cfName_).updateToken(
        StorageService.hash("503545744:0"));
    System.out.println("Done");

    /*
     * BigInteger hash = StorageService.hash("304700067:0"); List<Range> ranges
     * = new ArrayList<Range>(); ranges.add( new Range(new
     * BigInteger("1218069462158869448693347920504606362273788442553"), new
     * BigInteger("1092770595533781724218060956188429069")) ); if (
     * Range.isKeyInRanges(ranges, "304700067:0") ) {
     * System.out.println("Done"); }
     */
  }
}
