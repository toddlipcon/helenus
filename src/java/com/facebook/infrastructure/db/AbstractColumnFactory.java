...
abstract class AbstractColumnFactory {
  ...
  static {
    columnFactory_.put(ColumnFamily.getColumnType("Standard"),
        new ColumnFactory());
    columnFactory_.put(ColumnFamily.getColumnType("Super"),
        new SuperColumnFactory());
  }
  ...
  public IColumn createColumn(String name) {
    ...
    if (values.length == 0 || values.length > 2)
      throw new IllegalArgumentException(
          "Super Column "
              + name
              + " in invalid format. Must be in <super column name>:<column name> format.");
