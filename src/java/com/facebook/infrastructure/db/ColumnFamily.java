...
public final class ColumnFamily implements Serializable {
  ...
  /*
   * This method returns the serializer whose methods are preprocessed by a
   * dynamic proxy.
   */
  public static ICompactSerializer2<ColumnFamily> serializer2() {
    return (ICompactSerializer2<ColumnFamily>) Proxy.newProxyInstance(
        ColumnFamily.class.getClassLoader(),
        new Class[] { ICompactSerializer2.class },
        new CompactSerializerInvocationHandler<ColumnFamily>(serializer_));
  }
  ...
  private Comparator<IColumn> getColumnComparator(String cfName,
      String columnType) {
    if (columnComparator_ == null) {
      /*
       * if this columnfamily has supercolumns or there is an index on the
       * column name, then sort by name
       */
      if ("Super".equals(columnType)
          || DatabaseDescriptor.isNameIndexEnabled(cfName)) {
        columnComparator_ = ColumnComparatorFactory
            .getComparator(ColumnComparatorFactory.ComparatorType.NAME);
      }
      /*
       * if this columnfamily has simple columns, and no index on name sort by
       * timestamp
       */
      else {
        columnComparator_ = ColumnComparatorFactory
            .getComparator(ColumnComparatorFactory.ComparatorType.TIMESTAMP);
      }
    }
  ...
  /*
   * We need to go through each column in the column family and resolve it
   * before adding
   */
  ...
  /*
   * If we find an old column that has the same name the ask it to resolve
   * itself else add the new column .
   */
  ...
  /*
   * This is used as oldCf.merge(newCf). Basically we take the newCf and merge
   * it into the oldCf.
   */
  ...
  /*
   * This function will repair a list of columns If there are any columns in the
   * external list which are not present internally then they are added ( this
   * might have to change depending on how we implement delete repairs). Also if
   * there are any columns in teh internal and not in the external they are kept
  ...
  void repair(ColumnFamily columnFamily) {
    ...
    for (String cName : cNames) {
      ...
      if (columnInternal == null) {
        ...
        if (DatabaseDescriptor.getColumnFamilyType(name_).equals(
            ColumnFamily.getColumnType("Standard"))) {
  ...
  /*
   * This function will calculate the differnce between 2 column families the
   * external input is considered the superset of internal so there are no
   * deletes in the diff.
   */
...
class ColumnFamilySerializer implements ICompactSerializer2<ColumnFamily> {
  /*
   * We are going to create indexes, and write out that information as well. The
   * format of the data serialized is as follows.
   * 
   * 1) Without indexes: // written by the data <boolean false (index is not
   * present)> <column family id> <is marked for delete> <total number of
   * columns> <columns data>
   * 
   * <boolean true (index is present)>
   * 
   * This part is written by the column indexer <size of index in bytes> <list
   * of column names and their offsets relative to the first column>
   * 
   * <size of the cf in bytes> <column family id> <is marked for delete> <total
   * number of columns> <columns data>
   */
   ...
   */
  private ColumnFamily defreezeColumnFamily(DataInputStream dis)
      throws IOException {
  ...
  /*
   * This method fills the Column Family object with the column information from
   * the DataInputStream. The "items" parameter tells us whether we need all the
   * columns or just a subset of all the Columns that make up the Column Family.
   * If "items" is -1 then we need all the columns if not we deserialize only as
   * many columns as indicated by the "items" parameter.
   */
  private void fillColumnFamily(ColumnFamily cf, DataInputStream dis)
      throws IOException {
  ...
  /*
   * This version of deserialize is used when we need a specific set if columns
   * for a column family specified in the name cfName parameter.
   */
  public ColumnFamily deserialize(DataInputStream dis, IFilter filter)
      throws IOException {
    if (dis.available() == 0)
      return null;
  ...
  /*
   * Deserialize a particular column or super column or the entire columnfamily
   * given a : seprated name name could be of the form cf:superColumn:column or
   * cf:column or cf
   */
  public ColumnFamily deserialize(DataInputStream dis, String name,
      IFilter filter) throws IOException {
    if (dis.available() == 0)
      return null;
    ...
    if (!cf.isMarkedForDelete()) {
      ...
      int size = dis.readInt();
      for (int i = 0; i < size; ++i) {
        IColumn column = cf.getColumnSerializer().deserialize(dis, columnName,
            filter);
        if (column != null) {
          cf.addColumn(column.name(), column);
          break;
