...
class CommitLogHeader {
  ...
  static int size(int size) {
    /*
     * We serialize the CommitLogHeader as a byte[] and write it to disk. So we
     * first write an "int" to specify the length of the byte[] which is why we
     * first have a 4 in the sum. We then have size which is the number of bits
     * to track who has been flushed and then the rest is the position[] size =
     * #of column families + size of the bitset + size of position array
     */
  ...
  /*
   * Bitwise & of each byte in the two arrays. Both arrays are of same length.
   * In order to be memory efficient the result is in the third parameter.
   */
  ...
  /*
   * This ctor is used while deserializing. This ctor also builds an index of
   * position to column family Id.
   */
