...
public class DBManager {
  ...
  /*
   * Create the metadata tables. This table has information about the table name
   * and the column families that make up the table. Each column family also has
   * an associated ID which is an int.
   */
  private static void storeMetadata(
      Map<String, Set<String>> tableToColumnFamilyMap) throws Throwable {
    ...
    for (String table : tables) {
      ...
      if (tmetadata.isEmpty()) {
        ...
        for (String columnFamily : columnFamilies) {
          tmetadata.add(columnFamily, idGenerator.getAndIncrement(),
              DatabaseDescriptor.getColumnType(columnFamily));
        }
        ...
        tmetadata.add(Table.TableMetadata.cfName_, idGenerator
            .getAndIncrement());
        ...
        tmetadata.add(Table.hints_, idGenerator.getAndIncrement(), ColumnFamily
            .getColumnType("Super"));
        tmetadata.apply();
        idGenerator.set(0);
      }
  ...
  /*
   * This method reads the system table and retrieves the metadata associated
   * with this storage instance. Currently we store the metadata in a Column
   * Family called LocatioInfo which has two columns namely "Token" and
   * "Generation". This is the token that gets gossiped around and the
   * generation info is used for FD.
   */
  public DBManager.StorageMetadata start() throws IOException {
    ...
    if (row == null) {
      ...
      cf.addColumn(SystemTable.token_, new Column(SystemTable.token_, token
          .toByteArray()));
      cf.addColumn(SystemTable.generation_, new Column(SystemTable.generation_,
      ...
      for (String cfName : cfNames) {
        ...
        Column generation2 = new Column("Generation", BasicUtilities
            .intToByteArray(gen), generation.timestamp() + 1);
