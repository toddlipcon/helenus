...
public class Memtable implements MemtableMBean, Comparable<Memtable> {
  ...
  Memtable(String table, String cfName) throws IOException {
    if (apartments_.get(cfName) == null) {
      apartments_.put(cfName, new DebuggableThreadPoolExecutor(1, 1,
  ...
  boolean isThresholdViolated(String key) {
    ...
    if (currentSize_.get() >= threshold_
        || currentObjectCount_.get() >= thresholdCount_ || bVal
        || key.equals(flushKey_)) {
      if (bVal)
        logger_.info("Memtable's lifetime for " + cfName_
            + " has been violated.");
      return true;
    }
    return false;
  ...
  void printExecutorStats() {
    DebuggableThreadPoolExecutor es = (DebuggableThreadPoolExecutor) apartments_
        .get(cfName_);
   ...
   */
  void forceflush(ColumnFamilyStore cfStore, boolean fRecovery)
      throws IOException {
    if (!fRecovery) {
  ...
  /*
   * This version is called on commit log recovery. The threshold is not
   * respected and a forceFlush() needs to be invoked to flush the contents to
   * disk.
   */
    ...
    if (values.length == 1) {
      ...
      if (values.length == 2) {
        ...
        if (column != null) {
          ...
          if (subColumn != null) {
            columnFamily = new ColumnFamily(cfName_);
            columnFamily.createColumn(values[1] + ":" + values[2], subColumn
                .value(), subColumn.timestamp());
          }
        }
      }
    }
  ...
  /*
   * Although the method is named remove() we cannot remove the key from
   * memtable. We add it to the memtable but mark it as deleted. The reason for
   * this because we do not want a successive get() for the same key to scan the
   * ColumnFamilyStore files for this key.
   */
  ...
  /*
   * param recoveryMode - indicates if this was invoked during recovery.
   */
  void flush(CommitLog.CommitLogContext cLogCtx) throws IOException {
    ...
    if (columnFamilies_.size() == 0) {
      ...
      // even though there is nothing to flush in memtables for a given family
      // like Hints etc.
