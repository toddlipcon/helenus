...
/**
 * This class manages the persistence of checksums and keeps them in memory. It
 * maintains a mapping of data files on disk to their corresponding checksum
 * files. It is also loads the checksums in memory on start up.
 * 
 * @author alakshman
 * 
 */
class ChecksumManager {
  ...
  public static void onStart() throws IOException {
    ...
    for (File file : allFiles) {
      ...
      ChecksumReader chksumRdr = new ChecksumReader(file.getAbsolutePath(), 0L,
          file.length());
  ...
  /**
   * On delete of this dataFile remove the checksums associated with this file
   * from memory, remove the check sum manager instance.
   * 
   * @param dataFile
  ...
  public static void onFileDelete(String dataFile) throws IOException {
    ...
    String chksumFile = f.getParent() + System.getProperty("file.separator")
        + checksumPrefix_ + fileId + ".db";
  ...
  ChecksumManager(String dataFile) throws IOException {
    ...
    String chkSumFile = directory + System.getProperty("file.separator")
        + checksumPrefix_ + fId + ".db";
    raf_ = new RandomAccessFile(chkSumFile, "rw");
  }

  /**
   * Log the checksum for the the specified file and chunk within the file.
   * 
   * @param fileId
   ...
   * @param chksum
   *          calculated checksum for the chunk
   * @throws IOException
   */
  ...
  /**
   * Validate checksums for the data in the buffer for the region that is
   * encapsulated in the section object
   * 
   * @param bufOut
   *          buffer containing the data.
   * @param section
   *          coordinates indicating the region on disk.
   * @throws IOException
   */
  void validateChecksum(byte[] buffer, int chunkId, String file)
      throws IOException {
  ...
  public static void main(String[] args) throws Throwable {
    ChecksumReader rdr = new ChecksumReader(
        "C:\\Engagements\\Cassandra\\Checksum-1.db");
    while (!rdr.isEOF()) {
...
/**
 * ChecksumReader is used to memory map the checksum files and load the data
 * into memory.
 * 
 * @author alakshman
 * 
...
class ChecksumReader {
  ...
  public void map() throws IOException {
    ...
    try {
      buffer_ = file.getChannel().map(FileChannel.MapMode.READ_ONLY, 0,
          file.length());
      buffer_.load();
    } finally {
      file.close();
