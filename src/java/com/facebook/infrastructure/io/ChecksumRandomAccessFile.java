...
public final class ChecksumRandomAccessFile extends RandomAccessFile {
   ...
   * 
   * A file is said to bevalid if the following conditions hold:
   * 
   ...
   * disk(f)[i])
   * 
   * V5. "f.dirty" is true iff the buffer contains bytes that should be flushed
   * to the file; by V3 and V4, only part of the buffer can be dirty.
   * 
   ...
   * 
   * A file is said to beready if the buffer contains the current position,
   * i.e., when:
   * 
   * R1. !f.closed && f.buff != null && f.lo <= f.curr && f.curr < f.hi
   * 
   * When a file is ready, reading or writing a single byte can be performed by
   * reading or writing the in-memory buffer without performing a disk
   * operation.
   */
  ...
  public ChecksumRandomAccessFile(File file, String mode, int size)
      throws IOException {
    super(file, mode);
  ...
  public ChecksumRandomAccessFile(String name, String mode, int size)
   ...
   * number of bytes read. If the return result is less than "this.buff.length",
   * then EOF was read.
   */
  private int fillBuffer() throws IOException {
    ...
    if (n > 0) {
      int chunkId = getChunkId();
      // ChecksumManager.instance(filename_).validateChecksum(buff_, chunkId,
      // filename_);
    }
    this.diskPos_ += cnt;
    return cnt;
