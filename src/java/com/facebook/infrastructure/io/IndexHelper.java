...
/**
 * Provides helper to serialize, deserialize and use column indexes. Author :
 * Karthik Ranganathan ( kranganathan@facebook.com )
 */
...
public class IndexHelper {
   ...
   * 
   * @param indexSizeInBytes
   *          Size of index to be written
   * @param columnIndexList
   *          List of column index entries as objects
   * @param dos
   *          the output stream into which the column index is to be written
   * @throws IOException
   */
  public static void serialize(int indexSizeInBytes,
      List<ColumnPositionInfo> columnIndexList, DataOutputStream dos)
      throws IOException {
   ...
   * 
   * @param buffer
   *          the byte buffer from which the index should be skipped
   * @return number of bytes read from the data input
   * @throws IOException
   */
   ...
   * 
   * @param file
   *          Input from which the serialized form of the index is read
   * @param columnIndexList
   *          the structure which is filled in with the deserialized index
   * @return number of bytes read from the input
   * @throws IOException
   */
  static int deserializeIndex(RandomAccessFile file,
      List<ColumnPositionInfo> columnIndexList) throws IOException {
    /* read only the column index list */
    int columnIndexSize = file.readInt();
    ...
    while (indexIn.available() > 0) {
      ...
      columnIndexList
          .add(new ColumnPositionInfo(columnName, position, numCols));
    }
   ...
   *          Input from which the serialized form of the index is read
   * @param columnIndexList
   *          columnIndexList the structure which is filled in with the
   *          deserialized index
   * @return number of bytes read from the input
   * @throws IOException
   */
  static int deserializeIndex(ByteBuffer buffer,
      List<ColumnPositionInfo> columnIndexList) throws IOException {
    /* read only the column index list */
    int columnIndexSize = buffer.getInt();
    int totalBytesRead = 4;
   ...
   * 
   * @param column
   *          The column whose range needs to be found
   * @param columnIndexList
   *          the in-memory representation of the column index
   * @param dataSize
   ...
   */
  static ColumnPositionInfo getColumnRangeFromIndex(String column,
      List<ColumnPositionInfo> columnIndexList, int dataSize, int totalNumCols) {
    /*
     * if column indexes were not present for this column family, the handle
     * accordingly
     */
    ...
    long end = dataSize;
    int numColumns = 0;
    int index = Collections.binarySearch(columnIndexList, new KeyPositionInfo(
        column));
    if (index < 0) {
   ...
   * @param columnIndexList
   *          the deserialized column indexes
   * @param dataSize
   ...
   */
  static List<ColumnPositionInfo> getMultiColumnRangesFromIndex(
      List<String> columnNames, List<ColumnPositionInfo> columnIndexList,
      int dataSize, int totalNumCols) {
    ...
    for (String column : columnNames) {
      colPosInfo = getColumnRangeFromIndex(column, columnIndexList, dataSize,
          totalNumCols);
