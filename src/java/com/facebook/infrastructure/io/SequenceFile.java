...
/**
 * This class writes key/value pairs seqeuntially to disk. It is also used to
 * read sequentially from disk. However one could jump to random positions to
 * read data from the file. This class also has many implementations of the
 * IFileWriter and IFileReader interfaces which are exposed through factory
 * methods.
 * 
...
public class SequenceFile {
  ...
  public static class Writer extends AbstractWriter {
    ...
    public void append(DataOutputBuffer keyBuffer, DataOutputBuffer buffer)
      ...
      if (keyBuffer == null || keyBufLength == 0)
        throw new IllegalArgumentException(
            "Key cannot be NULL or of zero length.");
    ...
    /**
     * Be extremely careful while using this API. This currently used to write
     * the commit log header in the commit logs. If not used carefully it could
     * completely screw up reads of other key/value pairs that are written.
     * 
     * @param bytes
     *          the bytes to write
     */
  ...
  public static class ConcurrentWriter extends AbstractWriter {
    ...
    public void append(DataOutputBuffer keyBuffer, DataOutputBuffer buffer)
      ...
      /*
       * Size allocated "int" for key length + key + "int" for data length +
       * data
       */
      int length = buffer.getLength();
      ByteBuffer byteBuffer = ByteBuffer.allocateDirect(4 + keyBufLength + 4
          + length);
    ...
    public void append(String key, DataOutputBuffer buffer) throws IOException {
      ...
      ByteBuffer byteBuffer = ByteBuffer.allocateDirect(SequenceFile.utfPrefix_
          + key.length() + 4 + length);
    ...
    public void append(String key, byte[] value) throws IOException {
      ...
      ByteBuffer byteBuffer = ByteBuffer.allocateDirect(utfPrefix_
          + key.length() + 4 + value.length);
      SequenceFile.writeUTF(byteBuffer, key);
    ...
    public void close(byte[] footer, int size) throws IOException {
      ...
      ByteBuffer byteBuffer = ByteBuffer.allocateDirect(utfPrefix_
          + SequenceFile.marker_.length() + 4 + footer.length);
  ...
  public static abstract class AbstractReader implements IFileReader {
     ...
     * 
     * @param key
     *          the key whose offset is to be extracted from the current block
     *          index
     */
    public long getPositionFromBlockIndex(String key) throws IOException {
      ...
      for (int i = 0; i < keys; ++i) {
        ...
        if (keyInBlock.equals(key)) {
          ...
          /*
           * This is not the key we are looking for. So read its position and
           * the size of the data associated with it. This was strored as the
           * BlockMetadata.
           */
          bufIn.readLong();
          bufIn.readLong();
      ...
      /*
       * we do this because relative position of the key within a block is
       * stored.
       */
      if (position != -1L)
     ...
     */
    public SSTable.BlockMetadata getBlockMetadata(String key)
        throws IOException {
    ...
    /**
     * This function seeks to the position where the key data is present in the
     * file in order to get the buffer cache populated with the key-data. This
     * is done as a hint before the user actually queries the data.
     * 
     * @param key
     *          the key whose data is being touched
     * @param fData
     */
    public long touch(String key, boolean fData) throws IOException {
      ...
      if (keyInDisk != null) {
        /*
         * If key on disk is greater than requested key we can bail out since we
         * exploit the property of the SSTable format.
         */
        if (keyInDisk.compareTo(key) > 0)
          return bytesRead;

        /*
         * If we found the key then we populate the buffer that is passed in. If
         * not then we skip over this key and position ourselves to read the
         * next one.
         */
    ...
    /**
     * This method seek the disk head to the block index, finds the offset of
     * the key within the block and seeks to that offset.
     * 
     * @param key
     *          we are interested in.
     * @param section
     *          indicates the location of the block index.
     * @throws IOException
     */
     ...
     * Always use this method to query for application specific data as it will
     * have indexes.
     * 
     * @param key
    ...
    public long next(String key, DataOutputBuffer bufOut, String cf,
        Coordinate section) throws IOException {
      ...
      if (keyInDisk != null) {
        ...
        if (keyInDisk.equals(key)) {
          ...
          /*
           * if there is no column indexing enabled on this column then there
           * are no indexes for it
           */
          ...
          /*
           * if we need to read the all the columns do not read the column
           * indexes
           */
          else if (columnName == null) {
            int bytesSkipped = IndexHelper.skip(file_);
            /*
             * read the correct number of bytes for the column family and write
             * data into buffer
             */
            ...
            if (hasColumnIndexes) {
              ...
              /* read the index */
              totalBytesRead += IndexHelper.deserializeIndex(file_,
                  columnIndexList);
            }
            ...
            IndexHelper.ColumnPositionInfo columnRange = IndexHelper
                .getColumnRangeFromIndex(columnName, columnIndexList, dataSize,
                    totalNumCols);
             ...
             * returning: dataSize that we are reading + length of column family
             * name + one booleanfor deleted or not + one int for number of
             * columns
             */
     ...
     * @param columnNames
     *          The list of columns in the cfName column family that we want to
     *          return
     * @param section
     *          region of the file that needs to be read
     * @return total number of bytes read/considered
     ...
     */
    public long next(String key, DataOutputBuffer bufOut, String cf,
        List<String> columnNames, Coordinate section) throws IOException {
      String[] values = RowMutation.getColumnAndColumnFamily(cf);
      String columnFamilyName = values[0];
      ...
      if (keyInDisk != null) {
        ...
        if (keyInDisk.equals(key)) {
           ...
           */
            ...
            // TODO: this is name sorted - but eventually this should be sorted
            // by the same criteria as the col index
            /* sort the required list of columns */
            Collections.sort(columnNames);
            /* get the various column ranges we have to read */
            List<IndexHelper.ColumnPositionInfo> columnRangeList = IndexHelper
                .getMultiColumnRangesFromIndex(columnNames, columnIndexList,
                    dataSize, totalNumCols);
             ...
             * columns
             */
            bufOut.writeInt(dataSizeReturned + utfPrefix_ + cfName.length() + 4
                + 1);
            /* write the column family name */
            bufOut.writeUTF(cfName);
            /* write if this cf is marked for delete */
    ...
    public long next(DataOutputBuffer bufOut) throws IOException {
      ...
      /*
       * If we have read the bloom filter in the data file we know we are at the
       * end of the file and no further key processing is required. So we return
       * -1 indicating we are at the end of the file.
       */
  ...
  public static class Reader extends AbstractReader {
    ...
    /**
     * Be extremely careful while using this API. This currently used to read
     * the commit log header from the commit logs. Treat this as an internal
     * API.
     * 
     * @param bytes
     *          read from the buffer into the this array
     */
  ...
  public static IFileWriter concurrentWriter(String filename)
      throws IOException {
  ...
  /**
   * Efficiently writes a UTF8 string to the buffer. Assuming all Strings that
   * are passed in have length that can be represented as a short i.e length of
   * the string is <= 65535
   * 
   * @param buffer
   *          buffer to write the serialize version into
   * @param str
   *          string to serialize
   */
   ...
   * 
   * @param buffer
   *          buffer from which a UTF8 string is read
