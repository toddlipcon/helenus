...
public class Cachetable<K, V> implements ICachetable<K, V> {
  ...
  private void init(long expiration) {
    if (expiration <= 0)
      throw new IllegalArgumentException(
          "Argument specified must be a positive number");
  ...
  /*
   * Specify the TTL for objects in the cache in milliseconds.
   */
  ...
  /*
   * Specify the TTL for objects in the cache in milliseconds and a global
   * expunge hook. If a key has a key-specific hook installed invoke that
   * instead.
   */
