...
public class BloomFilter implements Serializable {
  public static class CountingBloomFilter implements Serializable {
    ...
    public BloomFilter.CountingBloomFilter merge(
        BloomFilter.CountingBloomFilter cbf) {
      if (cbf == null)
        return this;
    ...
    public void add(String key) {
      ...
      for (int i = 0; i < hashes_; ++i) {
        ...
        int hashValue = hash.hash(key);
        int index = Math.abs(hashValue % size_);
        byte value = (filter_[index] == 0xFF) ? filter_[index]
            : (byte) ((++filter_[index]) & 0xFF);
        filter_[index] = value;
      }
    }
    ...
    public boolean delete(String key) {
      ...
      for (int i = 0; i < hashes_; ++i) {
        ...
        int hashValue = hash.hash(key);
        int index = Math.abs(hashValue % size_);
        byte value = (filter_[index] == 0) ? filter_[index]
            : (byte) ((--filter_[index]) & 0xFF);
        filter_[index] = value;
      }
  ...
  public static void main(String[] args) throws Throwable {
    BloomFilter.CountingBloomFilter cbf = new BloomFilter.CountingBloomFilter(
        1024, 8);
    ...
    /*
     * BloomFilter.CountingBloomFilter cbf2 = new
     * BloomFilter.CountingBloomFilter(6410241024 + 1, 8); for (int i = 0; i <
     * 1025; ++i) { cbf2.add(Integer.valueOf(i).toString()); }
     * 
     * for (int i = 0; i < 1025; ++i) { boolean bVal =
     * cbf.isPresent(Integer.valueOf(i).toString()); if (!bVal)
     ...
     * 
     * for (int i = 0; i < 1025; ++i) { boolean bVal =
     * mbf.isPresent(Integer.valueOf(i).toString()); if (!bVal)
     * System.out.println(Integer.valueOf(i).toString()); }
     */
  }
}
...
class CountingBloomFilterSerializer implements
    ICompactSerializer<BloomFilter.CountingBloomFilter> {
  /*
   * The following methods are used for compact representation of BloomFilter.
   * This is essential, since we want to determine the size of the serialized
   * Bloom Filter blob before it is populated armed with the knowledge of how
  ...
  public void serialize(BloomFilter.CountingBloomFilter cbf,
  ...
  public BloomFilter.CountingBloomFilter deserialize(DataInputStream dis)
      throws IOException {
    /* read the size of the bloom filter */
    int size = dis.readInt();
    /* read the number of hash functions */
