...
public class BloomCalculations {
  ...
  private static final int[] optKPerBits = new int[] { 1, // dummy K for 0 bits
                                                          // per element
   ...
   * cell (i,j) the false positive rate determined by using i bits per element
   * and j hash functions.
   */
  private static final double[][] probs = new double[][] {
      ...
      { 1.0, 0.0689, 0.0177, 0.00718, 0.00381, 0.00244, 0.00179, 0.00146,
          0.00129 },
      { 1.0, 0.0645, 0.0156, 0.00596, 0.003, 0.00183, 0.00128, 0.001, 0.000852 } }; // the
                                                                                    // first
                                                                                    // column
                                                                                    // is
                                                                                    // a
  ...
  public static double getFailureRate(int bitsPerElement) {
    ...
    if (bitsPerElement >= probs.length)
      bitsPerElement = probs.length - 1;
   ...
   * minimizing bits per element rather than number of hash funtions.
   * 
  ...
  public static BloomSpecification computeBitsAndK(double maxFalsePosProb) {
    ...
    if (maxFalsePosProb >= probs[minBits][minK])
      return spec;
