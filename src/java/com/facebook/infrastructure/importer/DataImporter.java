...
public class DataImporter {
  ...
  public static EndPoint[] tos_ = new EndPoint[] {
      new EndPoint("hadoop038.sf2p.facebook.com", 7000),
  ...
  public Cassandra.Client connect() {
    ...
    String[] hosts = new String[] { "hadoop038.sf2p.facebook.com",
        "hadoop039.sf2p.facebook.com", "hadoop040.sf2p.facebook.com",
        "hadoop041.sf2p.facebook.com" };
    int port = 9160;

    // TNonBlockingSocket socket = new TNonBlockingSocket(hosts[roundRobin_],
    // port);
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      if (firstuser == null || firstuser.compareTo(nextuser) != 0) {
        ...
        if (rmInbox != null && !rmInbox.cfmap.isEmpty()) {
          /*
           * fos_.write(rmInbox.key.getBytes()); fos_.write(
        ...
        if (rmOutbox != null && !rmOutbox.cfmap.isEmpty()) {
          /*
           * fos_.write(rmOutbox.key.getBytes()); fos_.write(
           * System.getProperty("line.separator").getBytes());
           * counter_.incrementAndGet();
           */apply(rmOutbox);
        }
      ...
      if (folder == 0) {
        List<column_t> list = rmInbox.cfmap.get("MailboxUserList"
            + (columnFamilyHack_ % divideby_));
        if (list == null) {
          list = new ArrayList<column_t>();
          rmInbox.cfmap.put(
              "MailboxUserList" + (columnFamilyHack_ % divideby_), list);
        }
        // if(list.size() < 500)
        list.add(columnData);
      } else {
        List<column_t> list = rmOutbox.cfmap.get("MailboxUserList"
            + (columnFamilyHack_ % divideby_));
        if (list == null) {
          list = new ArrayList<column_t>();
          rmOutbox.cfmap.put("MailboxUserList"
              + (columnFamilyHack_ % divideby_), list);
        }
        // if(list.size() < 500)
        list.add(columnData);
      }
  ...
  public void testMailboxBatchRunner(String filepath) throws IOException {
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      columnData.value = String.valueOf(isDeleted);
      columnData.timestamp = lastUpdated;
      List<column_t> list = rmInbox.cfmap.get("MailboxMailList"
          + (columnFamilyHack_ % divideby_));
      if (list == null) {
        list = new ArrayList<column_t>();
        rmInbox.cfmap.put("MailboxMailList" + (columnFamilyHack_ % divideby_),
            list);
      }
      list.add(columnData);
    }
  ...
  public void testSuperBatchRunner(String filepath) throws IOException {
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      // List <MboxStruct> list = userthreadmap.get(rs.getString(1));
      if (folder == 0) {
        List<superColumn_t> list = rmInbox.cfmap.get("MailboxThreadList"
            + (columnFamilyHack_ % divideby_));
        if (list == null) {
          list = new ArrayList<superColumn_t>();
          rmInbox.cfmap.put("MailboxThreadList"
              + (columnFamilyHack_ % divideby_), list);
        }
        if (subject == null)
          subject = "";
        ...
        List<superColumn_t> list = rmOutbox.cfmap.get("MailboxThreadList"
            + (columnFamilyHack_ % divideby_));
        if (list == null) {
          list = new ArrayList<superColumn_t>();
          rmOutbox.cfmap.put("MailboxThreadList"
              + (columnFamilyHack_ % divideby_), list);
        }
        if (subject == null)
          subject = "";
  ...
  public void testSuperUserBatchRunner(String filepath) throws IOException {
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      // List <MboxStruct> list = userthreadmap.get(rs.getString(1));
      if (folder == 0) {
        List<superColumn_t> list = rmInbox.cfmap.get("MailboxUserList"
            + (columnFamilyHack_ % divideby_));
        if (list == null) {
          list = new ArrayList<superColumn_t>();
          rmInbox.cfmap.put(
        ...
        List<superColumn_t> list = rmOutbox.cfmap.get("MailboxUserList"
            + (columnFamilyHack_ % divideby_));
        if (list == null) {
          list = new ArrayList<superColumn_t>();
          rmOutbox.cfmap.put("MailboxUserList"
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      if (row == null) {
        ...
        if (cfMap == null || cfMap.size() == 0) {
          logger_.debug("ERROR ColumnFamil map is missing.....: " + threadId
              + "   key:" + key + "    record:" + line);
          System.out.println("ERROR ColumnFamil map is missing.....: "
              + threadId + "   key:" + key + "    record:" + line);
          errorCount_++;
          continue;
        }
        ColumnFamily cfamily = cfMap.get(columnFamily_);
        if (cfamily == null) {
          logger_.debug("ERROR ColumnFamily  is missing.....: " + threadId
              + "   key:" + key + "    record:" + line);
          System.out.println("ERROR ColumnFamily  is missing.....: " + threadId
              + "   key:" + key + "    record:" + line);
          errorCount_++;
          continue;
        }
        ...
        if (clmn == null) {
          logger_.debug("ERROR Column is missing.....: " + threadId
              + "    record:" + line);
          System.out.println("ERROR Column is missing.....: " + threadId
              + "    record:" + line);
          Thread.sleep(1000 / requestsPerSecond_, 1000 % requestsPerSecond_);
          errorCount_++;
        } else {
  ...
  private List<String> tokenize(String string) {
    ...
    TokenStream ts = analyzer.tokenStream("superColumn", new StringReader(
        string));
    Token token = null;
    try {
      token = ts.next();
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      if (firstuser == null || firstuser.compareTo(nextuser) != 0) {
        ...
        try {
          ...
          columns = peerstorageClient_.get_slice(tablename_, key, columnFamily_
              + (columnFamilyHack_ % divideby_), 0, 10);
          numReqs_++;
          totalTime_ = totalTime_ + (System.currentTimeMillis() - t);
          logger_.debug("Numreqs:" + numReqs_ + " Average: " + totalTime_
              / numReqs_ + "   Time taken for thrift..."
              + (System.currentTimeMillis() - t));
        } catch (Exception e) {
          e.printStackTrace();
        }
  ...
  public void testSuperReadThrift(String filepath) throws Throwable {
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      try {
        ...
        for (String token : tokenList) {
          long t = System.currentTimeMillis();
          columns = peerstorageClient_.get_slice(tablename_, key,
              "MailboxThreadList" + (columnFamilyHack_ % divideby_) + ":"
                  + token, 0, 10);
          totalTime_ = totalTime_ + (System.currentTimeMillis() - t);
          numReqs_++;
          logger_.debug("Numreqs:" + numReqs_ + " Average: " + totalTime_
              ...
              + (System.currentTimeMillis() - t));
          if (columns == null) {
            logger_.debug(" TOKEN: " + token
                + "  ERROR No row for this key .....: " + line);
            Thread.sleep(1000 / requestsPerSecond_, 1000 % requestsPerSecond_);
            errorCount_++;
          } else {
            if (columns.size() == 0) {
              logger_.debug("ERROR ColumnFamil map is missing.....: "
                  + threadId + "   key:" + key + " TOKEN: " + token
                  + "    record:" + line);
              System.out.println("ERROR ColumnFamil map is missing.....: "
                  + threadId + "   key:" + key + " TOKEN: " + token
                  + "    record:" + line);
  ...
  public void testLoadGeneratorBatchRunner(String filepath) throws Throwable {
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      // List <MboxStruct> list = userthreadmap.get(rs.getString(1));
      if (folder == 0) {
        rmInbox.add(columnFamily_ + (columnFamilyHack_ % divideby_) + ":"
            + threadId, String.valueOf(isDeleted).getBytes(), lastUpdated);
      } else {
        rmOutbox.add(columnFamily_ + (columnFamilyHack_ % divideby_) + ":"
            + threadId, String.valueOf(isDeleted).getBytes(), lastUpdated);
      }
    }
    if (firstuser != null) {
  ...
  public void applyLoad(RowMutation rm) throws IOException {
    try {
      ...
      Message message = new Message(to, StorageService.mutationStage_,
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      if (firstuser == null || firstuser.compareTo(nextuser) != 0) {
        ...
        try {
          ...
          peerstorageClient_.remove(tablename_, key,
              (columnFamilyHack_ % divideby_) + ":" + threadId);
          numReqs_++;
          totalTime_ = totalTime_ + (System.currentTimeMillis() - t);
          logger_.debug("Numreqs:" + numReqs_ + " Average: " + totalTime_
              / numReqs_ + "   Time taken for thrift..."
    ...
    if (args[0].compareTo("-testWriteMailbox") == 0
      ...
      for (int i = start; i < end; i++) {
        ...
        if (args[0].compareTo("-testWrite") == 0) {
          testBatchRunner(args[2] + System.getProperty("file.separator")
              + fileName);
        }
        if (args[0].compareTo("-testWriteSuper") == 0) {
          testSuperBatchRunner(args[2] + System.getProperty("file.separator")
              + fileName);
        }
        if (args[0].compareTo("-testReadSuper") == 0) {
          testSuperReadThrift(args[2] + System.getProperty("file.separator")
              + fileName);
        }
        if (args[0].compareTo("-testRemove") == 0) {
          testRemove(args[2] + System.getProperty("file.separator") + fileName);
        }
        ...
        if (args[0].compareTo("-testWriteMailbox") == 0) {
          testMailboxBatchRunner(args[2] + System.getProperty("file.separator")
              + fileName);
        }
        System.out.println(args[2] + System.getProperty("file.separator")
            + fileName);
      }
      if (transport_ != null)
        transport_.close();
  ...
  public void testPhp(String filepath) throws IOException {
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      String cmd = "php /home/pmalik/www/scripts/mbox_index/search_test.php  "
          + (new File(filepath)).getName() + "  " + user + "  " + threadId
          + "  " + line;
