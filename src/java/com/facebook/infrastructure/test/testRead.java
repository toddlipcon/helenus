...
public class testRead {
  static long read(String sKey, String sColumnFamily, String sColumn)
      throws Exception {
    long timeDelta = 0;
    ...
    try {
      ...
      // System.out.println("Read: TIME = " + timeDelta + "ms, key = " + sKey +
      // ", column family = " + sColumnFamily + ", column = " + sColumn +
      // ", data = " + new String(column.value()));
    } catch (Exception e) {
      e.printStackTrace();
      throw e;
    }
  ...
  static void readSimpleCols(String columnFamily, String key, int numCols) {
    ...
    System.out.println("Average read for " + columnFamily + ": "
        + (readTimeSum / numRuns));
  }
  ...
  static void readSuperCols(String columnFamily, String key, int numCols,
      int numSubCols) {
    Random random = new Random();
    int numRuns = 1000;
    String colName;
    ...
    readTimeSum = 0;
    for (int i = 0; i < numRuns; ++i) {
      colName = "SuperColumn-" + random.nextInt(numCols) + ":" + "Column-"
          + random.nextInt(numSubCols);
      try {
        readTimeSum += read(key, columnFamily, colName);
      } catch (Exception e) {
        e.printStackTrace();
