...
class ConsistencyManager implements Runnable {
  ...
  class DigestResponseHandler implements IAsyncCallback {
    ...
    private void doReadRepair() throws IOException {
      ...
      ReadMessage readMessage = new ReadMessage(table, row_.key(),
          columnFamily_);
  ...
  class DataRepairHandler implements IAsyncCallback,
    ...
    DataRepairHandler(int responseCount,
    ...
    public void response(Message message) {
      logger_.debug("Received responses in DataRepairHandler : "
          + message.toString());
    ...
    private void handleResponses() {
      try {
        ...
        logger_
            .info("We should not be coming here under any circumstances ...");
        logger_.info(LogUtil.throwableToString(ex));
      }
    }
  }
  ...
  private static ICachetable<String, String> readRepairTable_ = new Cachetable<String, String>(
      scheduledTimeMillis_);
  private Row row_;
  ...
  public void run() {
    ...
    if (columnNames_.size() == 0) {
      if (start_ >= 0 && count_ < Integer.MAX_VALUE) {
        readMessageDigestOnly = new ReadMessage(table, row_.key(),
    ...
    try {
      Message messageDigestOnly = ReadMessage
          .makeReadMessage(readMessageDigestOnly);
