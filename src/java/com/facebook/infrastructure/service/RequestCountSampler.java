...
public final class RequestCountSampler {
  ...
  private static ExecutorService cFlusher_ = new DebuggableThreadPoolExecutor(
      1, 1, Integer.MAX_VALUE, TimeUnit.SECONDS,
  ...
  public static class CardinalitySerializer implements
      ICompactSerializer<Cardinality> {
    /*
     * The following methods are used for compact representation of BloomFilter.
     * This is essential, since we want to determine the size of the serialized
     * Bloom Filter blob before it is populated armed with the knowledge of how
     * many elements are going to reside in it.
     */

    public void serialize(Cardinality cardinality, DataOutputStream dos)
        throws IOException {
      /* write out the CountinBloomFilter */
      BloomFilter.CountingBloomFilter.serializer().serialize(
          cardinality.bloomFilter_, dos);
      /* write the count of the # of keys */
      dos.writeInt(cardinality.count_);
    }

    public RequestCountSampler.Cardinality deserialize(DataInputStream dis)
        throws IOException {
      /* read the BloomFilter */
      BloomFilter.CountingBloomFilter cbf = BloomFilter.CountingBloomFilter
  ...
  private String getFileName() {
    ...
    return DatabaseDescriptor.getMetadataDirectory()
        + System.getProperty("file.separator") + table + "-Cardinality.db";
  }
  ...
  /*
   * Add the key passed in to the Counting Bloom Filter instance of the
   * Cardinality class.
   */
  ...
  void diseminateLoadInfo() {
    ...
    Gossiper.instance().addApplicationState(RequestCountSampler.loadInfo_,
        new ApplicationState(lInfo.toString()));
  }

  /*
   * Delete the key passed in from the Counting Bloom Filter instance of the
   * Cardinality class.
   */
  ...
  /*
   * Loop through the bitset and determine the number of bits that have been
   * flipped to one. This tells us the approximate number of keys that have been
   * processed. Approximate because we could have two distinct keys whose
   * intValue() could be the same. I need to determine the probability of such
   * an occurance.
   */
  int count() {
  ...
  /**
   * This method is called to update the cardinality of the of this node on load
   * balance/bootstrap operation.
   */
