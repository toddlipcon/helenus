...
public class HttpRequestVerbHandler implements IVerbHandler {
  private static final Logger logger_ = Logger
      .getLogger(HttpRequestVerbHandler.class);
  ...
  public void doVerb(Message message) {
    HttpConnection.HttpRequestMessage httpRequestMessage = (HttpConnection.HttpRequestMessage) message
        .getMessageBody()[0];
    try {
  ...
  private void doGet(HttpRequest httpRequest, HttpWriteResponse httpResponse) {
    ...
    /*
     * we do not care about the path for most requests except those from the
     * load balancer
     */
  ...
  /*
   * As a result of the POST query, we currently only send back some javascript
   * that updates the data in some place on the browser.
   */
  ...
  private String handleNodeDetails() {
    ...
    formatter.appendLine("Heap memory usage (in MB): " + smemUsed + "/"
        + smemMax);
    ...
    formatter
        .appendLine("<button onClick=\"window.location='"
            + StorageService.getHostUrl() + "?" + LOADME
            + "=T'\">Load Me</button>");
    formatter.appendLine("<button onClick=\"window.location='"
        + StorageService.getHostUrl() + "?" + COMPACTME
        + "=T'\">Compact Me</button>");
    formatter
        .appendLine("<button onClick=\"window.location='"
            + StorageService.getHostUrl() + "?" + KILLME
            + "=T'\">Kill Me</button>");

    formatter.appendLine("<br>");
    formatter.appendLine("<br><a href='" + StorageService.getHostUrl()
        + "'>Back to live nodes list" + "</a>");

    return formatter.toString();
  }

  private void displayDBStatistics(HTMLFormatter formatter,
      java.text.DecimalFormat df) {
    String tableStats = Table.open(DatabaseDescriptor.getTables().get(0))
        .tableStats("\n<br>\n", df);
  ...
  private String handlePageDisplay(String queryFormData, String insertFormData,
      String scriptFormData) {
    StringBuilder sb = new StringBuilder();
    sb.append("\n<div id=\"header\"> \n");
    sb.append("<ul>\n");
    sb
        .append("	<li name=\"one\" onclick=\"javascript:selectTab('one')\"><a href=\"#\">Cluster</a></li>\n");
    sb
        .append("	<li name=\"two\" onclick=\"javascript:selectTab('two')\"><a href=\"#\">SQL</a></li>\n");
    sb
        .append("	<li name=\"three\" onclick=\"javascript:selectTab('three')\"><a href=\"#\">Ring</a></li>\n");
    sb.append("</ul>\n");
    sb.append("</div>\n\n");
    ...
    sb
        .append("<div name=\"three\" id=\"content\"> <!-- start tab three -->\n\n");
  ...
  private String serveSummary() {
    ...
    String[] sHeaders = { "Node No.", "Host:Port", "Status", "Leader",
        "Load Info", "Token", "Generation No." };
    ...
    for (EndPoint curNode : liveNodes) {
      ...
      // Host:Port
      formatter.addCol("<a href='http://" + curNode.getHost() + ":"
          + DatabaseDescriptor.getHttpPort() + "/home?" + DETAILS + "=T'>"
          + curNode.getHost() + ":" + curNode.getPort() + "</a>");
      // Status
      String status = (FailureDetector.instance().isAlive(curNode)) ? "Up"
          : "Down";
      ...
      // Generation Number
      formatter.addCol(Integer.toString(Gossiper.instance()
          .getCurrentGenerationNumber(curNode)));
  ...
  private String serveRingView() {
    ...
    Map<Range, List<EndPoint>> oldRangeToEndPointMap = StorageService
        .instance().getRangeToEndPointMap();
  ...
  private String serveQueryForm(String queryResult) {
    ...
    formatter.appendLine("<FORM action=\"" + StorageService.getHostUrl()
        + "/home?" + QUERY + "=T\" method=\"post\">");
    ...
    formatter.appendLine("<BR>");
    formatter
        .appendLine("<INPUT type=\"submit\" value=\"Send\"> <INPUT type=\"reset\">");
  ...
  private String serveGroovyForm(String scriptResult) {
    HTMLFormatter formatter = new HTMLFormatter();
    formatter
        .appendLine("<BR><fieldset><legend>Run custom code on the cluster</legend>");
    formatter.appendLine("<FORM action=\"" + StorageService.getHostUrl()
        + "/home?" + SCRIPT + "=T\" method=\"post\">");

    formatter.append("Groovy code to run on the server:<br>");
    formatter
        .append("<textarea name=scriptTextArea rows=\"10\" cols=\"100\"></textarea>");
    formatter.appendLine("<BR>");
    formatter
        .appendLine("<INPUT type=\"submit\" value=\"Send\"> <INPUT type=\"reset\">");
  ...
  private String serveInsertForm(String insertResult) {
    HTMLFormatter formatter = new HTMLFormatter();
    formatter
        .appendLine("<BR><fieldset>\n<legend>Insert data into the cluster</legend>\n");
    formatter.appendLine("<FORM action=\"" + StorageService.getHostUrl()
        + "/home?" + INSERT + "=T\" method=\"post\">");
  ...
  private String handleQuery(HttpRequest httpRequest) {
    ...
    try {
      ...
      if (cf == null) {
        sRetVal = "Key [" + sKey + "], column family [" + sColumnFamily
            + "] not found.";
      } else {
  ...
  private String handleInsert(HttpRequest httpRequest) {
    ...
    try {
      ...
      fInsertSuccess = true;
      sRetVal = "columnfamily=" + httpRequest.getParameter("columnfamily")
          + " key=" + httpRequest.getParameter("key") + " data="
  ...
  private String handleScript(HttpRequest httpRequest) {
    ...
    try {
      sRetVal = com.facebook.infrastructure.scripts.GroovyScriptRunner
  ...
  private String handleKillMe() {
    ...
    /*
     * The storage service will wait for a period of time to let the VIP know
     * that we are shutting down, then will perform an actual shutdown on a
     * separate thread.
     */
