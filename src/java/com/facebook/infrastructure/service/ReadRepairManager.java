...
class ReadRepairManager {
  ...
  /*
   * This is the internal class which actually implements the global hook fn
   * called by the readrepair manager
   */
  static class ReadRepairPerformer implements
    ...
    /*
     * The hook fn which takes the end point and the row mutation that needs to
     * be sent to the end point in order to perform read repair.
     */
  ...
  private ICachetable<String, Message> readRepairTable_ = new Cachetable<String, Message>(
      expirationTimeInMillis, new ReadRepairManager.ReadRepairPerformer());
  ...
  /*
   * This is the fn that should be used to scheule a read repair specify a
   * endpoint on whcih the read repair should happen and the row mutaion message
   * that has the repaired row.
   */
  public void schedule(EndPoint target, RowMutationMessage rowMutationMessage) {
     ...
     * StorageService.mutationStage_, StorageService.readRepairVerbHandler_, new
     * Object[] { rowMutationMessage });
     */
    try {
      Message message = RowMutationMessage.makeRowMutationMessage(
          rowMutationMessage, StorageService.readRepairVerbHandler_);
