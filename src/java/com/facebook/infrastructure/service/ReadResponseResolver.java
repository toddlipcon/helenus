...
public class ReadResponseResolver implements IResponseResolver<Row> {
   ...
   * timestamp. For those columns where the timestamp is not the latest a repair
  ...
  public Row resolve(List<Message> responses) throws DigestMismatchException {
    ...
    /*
     * Populate the list of rows from each of the messages Check to see if there
     * is a digest query. If a digest query exists then we need to compare the
     * digest with the digest of the data that is received.
     */
    ...
    for (Message response : responses) {
      ...
      try {
        ...
        ReadResponseMessage result = ReadResponseMessage.serializer()
            .deserialize(bufIn);
    ...
    // If there is a mismatch then thwrow an exception so that read repair can
    // happen.
    if (isDigestQuery) {
      for (Row row : rowList) {
    ...
    for (int i = 0; i < rowList.size(); i++) {
      ...
      if (diffRow == null) // no repair needs to happen
        continue;
      // create the row mutation message based on the diff and schedule a read
      // repair
      ...
      RowMutationMessage rowMutationMessage = new RowMutationMessage(
          rowMutation);
