...
public class WriteResponseResolver implements IResponseResolver<Boolean> {
   ...
   * respones returned are false then we have a problem since that means the key
   * wa not written to any of the servers we want to notify the client of this
   * so in that case we should return a false saying that the write failed.
   */
  public Boolean resolve(List<Message> responses)
