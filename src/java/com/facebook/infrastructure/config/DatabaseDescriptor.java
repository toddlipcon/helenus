...
public class DatabaseDescriptor {
  ...
  /*
   * if the size of columns or super-columns are more than this, indexing will
   * kick in
   */
  ...
  /*
   * The compaction parameter decides if we should deserialize each key and then
   * serialize it to fix issues. true - we only deserialize if the key needs to
   * be resolved false - we always desrialize the key.
   */
  ...
  public static Map<String, Set<String>> init() throws Throwable {
    ...
    String file = System.getProperty("storage-config")
        + System.getProperty("file.separator") + "storage-conf.xml";
    return initInternal(file);
  }

  public static Map<String, Set<String>> initInternal(String file)
      throws Throwable {
    ...
    String touchKeyCacheSize = xmlUtils.getNodeValue(rootNode,
        "TouchKeyCacheSize");
    ...
    String columnIndexSizeInKB = xmlUtils.getNodeValue(rootNode,
        "ColumnIndexSizeInKB");
    ...
    /* data file directory */
    dataFileDirectories_ = xmlUtils.getNodeValues(rootNode,
        "DataFileDirectories/DataFileDirectory");
    ...
    bootstrapFileDirectory_ = xmlUtils.getNodeValue(rootNode,
    ...
    /* bootstrap file directory */
    stagingFileDirectory_ = xmlUtils.getNodeValue(rootNode,
    ...
    String value = xmlUtils.getNodeValue(rootNode,
    ...
    NodeList tables = xmlUtils.getRequestedNodeList(rootNode,
    ...
    for (int i = 0; i < size; ++i) {
      ...
      NodeList columnFamilies = xmlUtils.getRequestedNodeList(table,
          "ColumnFamily");
      ...
      for (int j = 0; j < size2; ++j) {
        ...
        String columnType = xmlUtils.getAttributeValue(columnFamily,
            "ColumnType");
        ...
        String columnIndexProperty = xmlUtils.getAttributeValue(columnFamily,
            "Index");
        String columnIndexType = ColumnFamily
            .getColumnIndexProperty(columnIndexProperty);
   ...
   * the disk with max free space for compactions. If the size of the expected
   * compacted file is greater than the max disk space available return null, we
   * cannot do compaction in this case.
   */
  public static String getCompactionFileLocation(long expectedCompactedFileSize) {
    ...
    // Load factor of 0.9 we do not want to use the entire disk that is too
    // risky.
