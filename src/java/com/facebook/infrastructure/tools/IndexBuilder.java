...
public class IndexBuilder {
  ...
  public static void main(String[] args) {
    if (args.length != 1) {
      System.out
          .println("Usage : java com.facebook.infrastructure.tools.IndexBuilder <full path to the data file>");
      System.exit(1);
    }

    try {
      int blockCount = getBlockCount(args[0]);
      System.out.println("Number of keys in the data file : "
          + (blockCount + 1) * SSTable.indexInterval());
  ...
  private static void buildIndex(String dataFile, int blockCount)
      throws IOException {
    String indexFile = dataFile.replace("-Data.", "-Index.");
    final int bufferSize = 64 * 1024;

    IFileWriter indexWriter = SequenceFile
        .bufferedWriter(indexFile, bufferSize);
    IFileReader dataReader = SequenceFile.bufferedReader(dataFile, bufferSize);
    DataOutputBuffer bufOut = new DataOutputBuffer();
    DataInputBuffer bufIn = new DataInputBuffer();
    /* BloomFilter of all data in the data file */
    BloomFilter bf = new BloomFilter(
        (SSTable.indexInterval() + 1) * blockCount, 8);
    ...
    try {
      while (!dataReader.isEOF()) {
        ...
        if (key.equals(SSTable.blockIndexKey_)) {
          ...
          /*
           * Read the keys in this block and find the largest key in this block.
           * This is the key that gets written into the index file.
           */
          for (int i = 0; i < blockSize; ++i) {
            ...
            /*
             * read the position of the key and the size of key data and throws
             * it away.
             */
            bufIn.readLong();
            bufIn.readLong();
          ...
          /*
           * Write into the index file the largest key in the block and the
           * offset of the block index in the data file.
           */
          indexWriter.append(largestKey, BasicUtilities
              .longToByteArray(blockIndexOffset));
        }
      }
    } finally {
      dataReader.close();
