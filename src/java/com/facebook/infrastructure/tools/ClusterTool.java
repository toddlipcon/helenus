...
public class ClusterTool {
  ...
  public static void applyToken(String serverName, BigInteger token)
      throws IOException {
    try {
      EndPoint from = new EndPoint(InetAddress.getLocalHost().getHostName(),
          7000);
      System.out.println("Updating token of server " + serverName
          + " with token " + token);
      Message message = new Message(from, "", StorageService.tokenVerbHandler_,
          new Object[] { token.toByteArray() });
  ...
  public static void printUsage() {
    System.out
        .println("Usage: java -jar <cassandra-tools.jar> <command> <options>");
    ...
    System.out
        .println("\t"
            + READ_TEST
            + " <number of threads> <requests per sec per thread> <machine(s) to read (':' separated list)>");
    System.out
        .println("\t"
            + WRITE_TEST
            + " <number of threads> <requests per sec per thread> <machine(s) to write (':' separated list)>");
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      ...
      if (SET_TOKEN.equals(args[argc])) {
        ...
        // System.out.println("Calibrating " + serverName + " with token " +
        // token);
