...
public class Gossiper implements IFailureDetectionEventListener,
    IEndPointStateChangePublisher, IComponentShutdown {
  private class GossipTimerTask extends TimerTask {
    public void run() {
      try {
        synchronized (Gossiper.instance()) {
          /* Update the local heartbeat counter. */
          endPointStateMap_.get(localEndPoint_).getHeartBeatState()
              .updateHeartBeat();
          ...
          if (gDigests.size() > 0) {
            ...
            /*
             * Gossip to some unreachable member with some probability to check
             * if he is back up
             */
  ...
  /*
   * map where key is the endpoint and value is the state associated with the
   * endpoint
   */
  ...
  Gossiper() {
    ...
    StageManager.registerStage(Gossiper.GOSSIP_STAGE, new SingleThreadedStage(
        "GMFD"));
  ...
  public Set<EndPoint> getLiveMembers() {
    ...
    liveMbrs.add(new EndPoint(localEndPoint_.getHost(), localEndPoint_
        .getPort()));
    return liveMbrs;
  }
  ...
  /**
   * This method is used to forcibly remove a node from the membership set. He
   * is forgotten locally immediately.
   * 
  ...
  public void convict(EndPoint endpoint) {
    ...
    if (epState != null) {
      if (!epState.isAlive() && epState.isAGossiper()) {
        /*
         * just to be sure - is invoked just to make sure that it was called
         * atleast once.
         */
        if (liveEndpoints_.contains(endpoint)) {
          ...
          EndPointState deltaState = new EndPointState(epState
              .getHeartBeatState());
  ...
  /**
   * This method is part of IFailureDetectionEventListener interface. This is
   * invoked by the Failure Detector when it suspects an end point.
   * 
  ...
  /*
   * No locking required since it is called from a method that already has
   * acquired a lock
   */
  @Deprecated
  ...
  /**
   * No locking required since it is called from a method that already has
   * acquired a lock. The gossip digest is built based on randomization rather
   * than just looping through the collection of live endpoints.
   * 
   * @param gDigests
  ...
  Message makeGossipDigestSynMessage(List<GossipDigest> gDigests)
      throws IOException {
    GossipDigestSynMessage gDigestMessage = new GossipDigestSynMessage(
        DatabaseDescriptor.getClusterName(), gDigests);
    ByteArrayOutputStream bos = new ByteArrayOutputStream(
        Gossiper.MAX_GOSSIP_PACKET_SIZE);
    DataOutputStream dos = new DataOutputStream(bos);
    GossipDigestSynMessage.serializer().serialize(gDigestMessage, dos);
    Message message = new Message(localEndPoint_, Gossiper.GOSSIP_STAGE,
        GOSSIP_DIGEST_SYN_VERB, new Object[] { bos.toByteArray() });
    return message;
  }
  ...
  Message makeGossipDigestAckMessage(GossipDigestAckMessage gDigestAckMessage)
    ...
    logger_.debug("@@@@ Size of GossipDigestAckMessage is "
        + bos.toByteArray().length);
    Message message = new Message(localEndPoint_, Gossiper.GOSSIP_STAGE,
        GOSSIP_DIGEST_ACK_VERB, new Object[] { bos.toByteArray() });
    return message;
  }
  ...
  Message makeGossipDigestAck2Message(GossipDigestAck2Message gDigestAck2Message)
    ...
    Message message = new Message(localEndPoint_, Gossiper.GOSSIP_STAGE,
        GOSSIP_DIGEST_ACK2_VERB, new Object[] { bos.toByteArray() });
    return message;
  }
  ...
  /*
   * Sends a Gossip message to a live member and returns a reference to the
   * member
   */
  ...
  void doGossipToSeed(Message message) {
    ...
    if (size > 0) {
      ...
      if (liveEndpoints_.size() == 0) {
        ...
        double probability = seeds_.size()
            / (liveEndpoints_.size() + unreachableEndpoints_.size());
        double randDbl = random_.nextDouble();
  ...
  synchronized EndPointState getStateForVersionBiggerThan(EndPoint forEndpoint,
      int version) {
    ...
    if (epState != null) {
      /*
       * Here we try to include the Heart Beat state only if it is greater than
       * the version passed in. It might happen that the heart beat version
       * maybe lesser than the version passed in and some application state has
       * a version that is greater than the version passed in. In this case we
       * also send the old heart beat and throw it away on the receiver if it is
       * redundant.
       */
  ...
  /*
   * This method is called only from the JoinVerbHandler. This happens when a
   * new node coming up multicasts the JoinMessage. Here we need to add the
   * endPoint to the list of live endpoints.
   */
  ...
  void notifyFailureDetector(List<GossipDigest> gDigests) {
    ...
    for (GossipDigest gDigest : gDigests) {
      EndPointState localEndPointState = endPointStateMap_
          .get(gDigest.endPoint_);
      /*
       * If the local endpoint state exists then report to the FD only if the
       * versions workout.
       */
      if (localEndPointState != null) {
        int localGeneration = endPointStateMap_.get(gDigest.endPoint_)
            .getHeartBeatState().generation_;
    ...
    for (EndPoint endpoint : endpoints) {
      ...
      if (localEndPointState != null) {
        ...
        if (remoteGeneration == localGeneration) {
          int localVersion = getMaxEndPointStateVersion(localEndPointState);
          // int localVersion =
          // localEndPointState.getHeartBeatState().getHeartBeatVersion();
          int remoteVersion = remoteEndPointState.getHeartBeatState()
              .getHeartBeatVersion();
          if (remoteVersion > localVersion) {
            logger_.debug("Reporting " + endpoint + " to the FD.");
            fd.report(endpoint);
  ...
  synchronized void applyStateLocally(Map<EndPoint, EndPointState> epStateMap) {
    ...
    for (EndPoint ep : eps) {
      ...
      /*
       * If state does not exist just add it. If it does then add it only if the
       * version of the remote copy is greater than the local copy.
       */
      if (localEpStatePtr != null) {
        int localGeneration = localEpStatePtr.getHeartBeatState()
            .getGeneration();
  ...
  void applyHeartBeatStateLocally(EndPoint addr, EndPointState localState,
    ...
    if (localHbState.getGeneration() == remoteHbState.getGeneration()) {
      if (remoteHbState.getHeartBeatVersion() > localHbState
          .getHeartBeatVersion()) {
        int oldVersion = localHbState.getHeartBeatVersion();
        localState.setHeartBeatState(remoteHbState);
        logger_.debug("Updating heartbeat state version to "
            + localState.getHeartBeatState().getHeartBeatVersion() + " from "
            + oldVersion + " for " + addr + " ...");
      }
    }
  }

  void applyApplicationStateLocally(EndPoint addr, EndPointState localStatePtr,
      EndPointState remoteStatePtr) {
    Map<String, ApplicationState> localAppStateMap = localStatePtr
        .getApplicationState();
    Map<String, ApplicationState> remoteAppStateMap = remoteStatePtr
        .getApplicationState();
    ...
    for (String remoteKey : remoteKeys) {
      ...
      if (localAppState == null) {
        ...
        EndPointState deltaState = new EndPointState(localStatePtr
            .getHeartBeatState());
      ...
      /*
       * If the remoteGeneration is greater than localGeneration then apply
       * state blindly
       */
      if (remoteGeneration > localGeneration) {
        localStatePtr.addApplicationState(remoteKey, remoteAppState);
      ...
      /*
       * If the generations are the same then apply state if the remote version
  ...
  Map<EndPoint, GossipDigest> getEndPointGossipDigestMap(
  ...
  void requestAll(GossipDigest gDigest,
      List<GossipDigest> deltaGossipDigestList, int remoteGeneration) {
    /*
     * We are here since we have no data for this endpoint locally so request
     * everthing.
     */
  ...
  void sendAll(GossipDigest gDigest,
      Map<EndPoint, EndPointState> deltaEpStateMap, int maxRemoteVersion) {
    EndPointState localEpStatePtr = getStateForVersionBiggerThan(gDigest
        .getEndPoint(), maxRemoteVersion);
    if (localEpStatePtr != null)
      deltaEpStateMap.put(gDigest.getEndPoint(), localEpStatePtr);
  }

  /*
   * This method is used to figure the state that the Gossiper has but Gossipee
   * doesn't. The delta digests and the delta state are built up.
   */
  synchronized void examineGossiper(List<GossipDigest> gDigestList,
      List<GossipDigest> deltaGossipDigestList,
      Map<EndPoint, EndPointState> deltaEpStateMap) {
    for (GossipDigest gDigest : gDigestList) {
      ...
      /*
       * Here we need to fire a GossipDigestAckMessage. If we have some data
       * associated with this endpoint locally then we follow the "if" path of
       * the logic. If we have absolutely nothing for this endpoint we need to
       * request all the data for this endpoint.
       */
      if (epStatePtr != null) {
        int localGeneration = epStatePtr.getHeartBeatState().getGeneration();
        /*
         * get the max version of all keys in the state associated with this
         * endpoint
         */
        ...
        if (remoteGeneration == localGeneration) {
           ...
           * endpoint send us all the data for this endpoint with version
           * greater than the max version number we have locally for this
           * endpoint. If the max remote version is lesser, then we send all the
           * data we have locally for this endpoint with version greater than
           * the max remote version.
           */
          if (maxRemoteVersion > maxLocalVersion) {
            deltaGossipDigestList.add(new GossipDigest(gDigest.getEndPoint(),
                remoteGeneration, maxLocalVersion));
          }
          if (maxRemoteVersion < maxLocalVersion) {
            /*
  ...
  public void start(EndPoint localEndPoint, int generationNbr)
      throws IOException {
    localEndPoint_ = localEndPoint;
    /* Get the seeds from the config and initialize them. */
    Set<String> seedHosts = DatabaseDescriptor.getSeeds();
    for (String seedHost : seedHosts) {
      EndPoint seed = new EndPoint(seedHost, DatabaseDescriptor
          .getControlPort());
    ...
    gossipTimer_.schedule(new GossipTimerTask(), Gossiper.intervalInMillis_,
        Gossiper.intervalInMillis_);
  }

  public void shutdown() {
    /* This prevents this guy from responding to Gossip messages */
    MessagingService.getMessagingInstance().deregisterVerbHandlers(
        GOSSIP_DIGEST_SYN_VERB);
    MessagingService.getMessagingInstance().deregisterVerbHandlers(
        GOSSIP_DIGEST_ACK_VERB);
    MessagingService.getMessagingInstance().deregisterVerbHandlers(
        GOSSIP_DIGEST_ACK2_VERB);
    /* This prevents this guy from Gossiping */
    gossipTimer_.cancel();
  }

  public synchronized void addApplicationState(String key,
...
class GossipDigestSynVerbHandler implements IVerbHandler {
  private static Logger logger_ = Logger
      .getLogger(GossipDigestSynVerbHandler.class);
  ...
  public void doVerb(Message message) {
    ...
    try {
      GossipDigestSynMessage gDigestMessage = GossipDigestSynMessage
          .serializer().deserialize(dis);
      /* If the message is from a different cluster throw it away. */
      if (!gDigestMessage.clusterId_
          .equals(DatabaseDescriptor.getClusterName()))
        return;
      ...
      GossipDigestAckMessage gDigestAck = new GossipDigestAckMessage(
          deltaGossipDigestList, deltaEpStateMap);
      Message gDigestAckMessage = Gossiper.instance()
          .makeGossipDigestAckMessage(gDigestAck);
      logger_.info("Sending a GossipDigestAckMessage to " + from);
      MessagingService.getMessagingInstance().sendUdpOneWay(gDigestAckMessage,
          from);
    } catch (IOException e) {
  ...
  /*
   * First construct a map whose key is the endpoint in the GossipDigest and the
   * value is the GossipDigest itself. Then build a list of version differences
   * i.e difference between the version in the GossipDigest and the version in
   * the local state for a given EndPoint. Sort this list. Now loop through the
   * sorted list and retrieve the GossipDigest corresponding to the endpoint
   * from the map that was initially constructed.
   */
  private void doSort(List<GossipDigest> gDigestList) {
    ...
    for (GossipDigest gDigest : gDigestList) {
      ...
      EndPointState epState = Gossiper.instance().getEndPointStateForEndPoint(
          ep);
      int version = (epState != null) ? Gossiper.instance()
          .getMaxEndPointStateVersion(epState) : 0;
      int diffVersion = Math.abs(version - gDigest.getMaxVersion());
      diffDigests
          .add(new GossipDigest(ep, gDigest.getGeneration(), diffVersion));
    }
...
class GossipDigestAckVerbHandler implements IVerbHandler {
  private static Logger logger_ = Logger
      .getLogger(GossipDigestAckVerbHandler.class);
  ...
  public void doVerb(Message message) {
    ...
    try {
      GossipDigestAckMessage gDigestAckMessage = GossipDigestAckMessage
          .serializer().deserialize(dis);
      List<GossipDigest> gDigestList = gDigestAckMessage.getGossipDigestList();
      Map<EndPoint, EndPointState> epStateMap = gDigestAckMessage
          .getEndPointStateMap();
      ...
      for (GossipDigest gDigest : gDigestList) {
        ...
        EndPointState localEpStatePtr = Gossiper.instance()
            .getStateForVersionBiggerThan(addr, gDigest.getMaxVersion());
        if (localEpStatePtr != null)
      ...
      GossipDigestAck2Message gDigestAck2 = new GossipDigestAck2Message(
          deltaEpStateMap);
      Message gDigestAck2Message = Gossiper.instance()
          .makeGossipDigestAck2Message(gDigestAck2);
      logger_.info("Sending a GossipDigestAck2Message to " + from);
      MessagingService.getMessagingInstance().sendUdpOneWay(gDigestAck2Message,
          from);
    } catch (IOException e) {
      logger_.info(LogUtil.throwableToString(e));
    }
...
class GossipDigestAck2VerbHandler implements IVerbHandler {
  private static Logger logger_ = Logger
      .getLogger(GossipDigestAck2VerbHandler.class);
  ...
  public void doVerb(Message message) {
    ...
    DataInputStream dis = new DataInputStream(new ByteArrayInputStream(bytes));
    try {
      GossipDigestAck2Message gDigestAck2Message = GossipDigestAck2Message
          .serializer().deserialize(dis);
      Map<EndPoint, EndPointState> remoteEpStateMap = gDigestAck2Message
          .getEndPointStateMap();
      /* Notify the Failure Detector */
