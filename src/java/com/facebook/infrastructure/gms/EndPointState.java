...
class EndPointStateSerializer implements ICompactSerializer<EndPointState> {
  private static Logger logger_ = Logger
      .getLogger(EndPointStateSerializer.class);

  public void serialize(EndPointState epState, DataOutputStream dos)
      throws IOException {
    ...
    if (size > 0) {
      ...
      for (String key : keys) {
        if (Gossiper.MAX_GOSSIP_PACKET_SIZE - dos.size() < estimate) {
          logger_
              .info("@@@@ Breaking out to respect the MTU size in EndPointState serializer. Estimate is "
                  + estimate + " @@@@");
          break;
        }
  ...
  public EndPointState deserialize(DataInputStream dis) throws IOException {
    ...
    for (int i = 0; i < appStateSize; ++i) {
      ...
      ApplicationState appState = ApplicationState.serializer()
          .deserialize(dis);
