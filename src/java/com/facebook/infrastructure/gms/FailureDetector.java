...
public class FailureDetector implements IFailureDetector, FailureDetectorMBean {
  ...
  public FailureDetector() {
    ...
    try {
      ...
      mbs.registerMBean(this, new ObjectName(
          "com.facebook.infrastructure.gms:type=FailureDetector"));
    } catch (Exception e) {
  ...
  /**
   * We dump the arrival window for any endpoint only if the local Failure
   * Detector module has been up for more than a minute.
   * 
   * @param ep
   *          for which the arrival window needs to be dumped.
   */
  private void dumpInterArrivalTimes(EndPoint ep) {
    ...
    try {
      FileOutputStream fos = new FileOutputStream("/var/tmp/output-"
          + System.currentTimeMillis() + "-" + ep + ".dat", true);
  ...
  public boolean isAlive(EndPoint ep) {
    ...
    /*
     * Incoming port is assumed to be the Storage port. We need to change it to
     * the control port
     */
    EndPoint ep2 = new EndPoint(ep.getHost(), DatabaseDescriptor
        .getControlPort());
    EndPointState epState = Gossiper.instance()
        .getEndPointStateForEndPoint(ep2);
  ...
  public void intepret(EndPoint ep) {
    ...
    /*
     * if ( phi > phiConvictThreshold_ ) { isConvicted = true; for (
     * IFailureDetectionEventListener listener : fdEvntListeners_ ) {
     * listener.convict(ep); } }
     */
...
class ArrivalWindow {
  ...
  synchronized double sumOfDeviations() {
    ...
    for (int i = 0; i < size; ++i) {
      sumOfDeviations += (arrivalIntervals_.get(i) - mean)
          * (arrivalIntervals_.get(i) - mean);
    }
