...
public class TcpConnection extends SelectionKeyHandler implements Comparable {
  ...
  /*
   * Added for streaming support. We need the boolean to indicate that this
   * connection is used for streaming. The Condition and the Lock are used to
   * signal the stream() that it can continue streaming when the socket becomes
   * writable.
   */
  ...
  TcpConnection(TcpConnectionManager pool, EndPoint from, EndPoint to)
      throws IOException {
    ...
    if (!socketChannel_.connect(remoteEp_.getInetAddress())) {
      key_ = SelectorManager.getSelectorManager().register(socketChannel_,
  ...
  /*
   * This method is invoked by the TcpConnectionHandler to accept incoming TCP
   * connections. Accept the connection and then register interest for reads.
   */
  static void acceptConnection(SocketChannel socketChannel, EndPoint localEp,
      boolean isIncoming) throws IOException {
    TcpConnection tcpConnection = new TcpConnection(socketChannel, localEp,
        true);
    tcpConnection.registerReadInterest();
  }

  private void registerReadInterest() throws IOException {
    key_ = SelectorManager.getSelectorManager().register(socketChannel_, this,
        SelectionKey.OP_READ);
  }

  // used for incoming connections
  TcpConnection(SocketChannel socketChannel, EndPoint localEp,
      boolean isIncoming) throws IOException {
  ...
  public void write(Message message) throws IOException {
    ...
    if (data.length > 0) {
      boolean listening = (message.getFrom()
          .equals(EndPoint.randomLocalEndPoint_)) ? false : true;
      ByteBuffer buffer = MessagingService
          .packIt(data, false, false, listening);
      synchronized (this) {
  ...
  public void stream(File file, long startPosition, long endPosition)
      throws IOException {
    if (!bStream_)
      throw new IllegalStateException(
          "Cannot stream since we are not set up to stream data.");
    ...
    try {
      ...
      /*
       * If the connection is not yet established then wait for the timeout
       * period of 2 seconds. Attempt to reconnect 3 times and then bail with an
       * IOException.
       */
      long waitTime = 2;
      int retry = 0;
      while (!connected_.get()) {
        if (retry == 3)
          throw new IOException("Unable to connect to " + remoteEp_ + " after "
              + retry + " attempts.");
      ...
      while (bytesWritten < total) {
        ...
        /*
         * If the number of bytes transferred is less than intended then we need
         * to wait till socket becomes writeable again.
         */
  ...
  void closeSocket() {
    logger_.warn("Closing down connection " + socketChannel_ + " with "
        + pendingWrites_.size() + " writes remaining.");
    if (pool_ != null) {
  ...
  public void connect(SelectionKey key) {
    ...
    try {
      if (socketChannel_.finishConnect()) {
        ...
        logger_
            .warn("Closing connection because socket channel could not finishConnect.");
        ;
        errorClose();
      }
    } catch (IOException e) {
  ...
  public void write(SelectionKey key) {
    ...
    doPendingWrites();
    /*
     * This is executed only if we are in streaming mode. Idea is that we read a
     * chunk of data from a source and wait to read the next from the source
     * until we are siganlled to do so from here.
     */
    resumeStreaming();
  }
  ...
  void doPendingWrites() {
    try {
      ...
      synchronized (this) {
        if (!pendingWrites_.isEmpty()
            && (key_.interestOps() & SelectionKey.OP_WRITE) == 0) {
          SelectorManager.getSelectorManager().modifyKeyForWrite(key_);
        }
      }
    }
  ...
  class ReadWorkItem implements Runnable {
    ...
    public void run() {
      if (tcpReader_ == null) {
        ...
        StartState nextState = tcpReader_
            .getSocketState(TcpReader.TcpReaderState.PREAMBLE);
        if (nextState == null) {
          nextState = new ProtocolState(tcpReader_);
          tcpReader_.putSocketState(TcpReader.TcpReaderState.PREAMBLE,
              nextState);
        }
      ...
      try {
        ...
        while ((bytes = tcpReader_.read()).length > 0) {
          ...
          if (!pH.isStreamingMode_) {
            ...
            if (remoteEp_ == null) {
              int port = (pH.isListening_) ? DatabaseDescriptor
                  .getStoragePort() : EndPoint.randomPort_;
              remoteEp_ = new EndPoint(socketChannel_.socket().getInetAddress()
                  .getHostName(), port);
