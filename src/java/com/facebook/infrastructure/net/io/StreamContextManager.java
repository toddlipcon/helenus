...
public class StreamContextManager {
  ...
  public static class StreamContext implements Serializable {
    private static Logger logger_ = Logger
        .getLogger(StreamContextManager.StreamContext.class);
    ...
    public StreamContext(String targetFile, long expectedBytes,
        RequestCountSampler.Cardinality cardinality) {
      targetFile_ = targetFile;
      expectedBytes_ = expectedBytes;
  ...
  public static class StreamContextSerializer implements
      ICompactSerializer<StreamContext> {
    public void serialize(StreamContextManager.StreamContext sc,
      ...
      if (sc.cardinality_ == null) {
        ...
        RequestCountSampler.Cardinality.serializer().serialize(sc.cardinality_,
            dos);
      }
    }

    public StreamContextManager.StreamContext deserialize(DataInputStream dis)
        throws IOException {
      ...
      if (bVal)
        cardinality = RequestCountSampler.Cardinality.serializer().deserialize(
            dis);
  ...
  public static class StreamStatusSerializer implements
    ...
    public void serialize(StreamStatus streamStatus, DataOutputStream dos)
        throws IOException {
  ...
  public static class StreamStatusMessage implements Serializable {
    ...
    public static Message makeStreamStatusMessage(
        StreamStatusMessage streamStatusMessage) throws IOException {
      ...
      return new Message(StorageService.getLocalStorageEndPoint(), "",
          StorageService.bootStrapTerminateVerbHandler_, new Object[] { bos
              .toByteArray() });
    }
  ...
  public static class StreamStatusMessageSerializer implements
    ...
    public void serialize(StreamStatusMessage streamStatusMessage,
        DataOutputStream dos) throws IOException {
      StreamStatus.serializer().serialize(streamStatusMessage.streamStatus_,
          dos);
    }

    public StreamStatusMessage deserialize(DataInputStream dis)
        throws IOException {
      StreamContextManager.StreamStatus streamStatus = StreamStatus
  ...
  /*
   * Maintain in this map the status of the streams that need to be sent back to
   * the source
   */
  public static Map<String, List<StreamStatus>> streamStatusBag_ = new Hashtable<String, List<StreamStatus>>();
  /*
   * Maintains a callback handler per endpoint to notify the app that a stream
   * from a given endpoint has been handled
   */
  ...
  /*
   * This method helps determine if the StreamCompletionHandler needs to be
   * invoked for the data being streamed from a source.
   */
  public synchronized static boolean isDone(String key) {
    return (ctxBag_.get(key) == null);
  }

  public synchronized static IStreamComplete getStreamCompletionHandler(
      String key) {
  ...
  public synchronized static void registerStreamCompletionHandler(String key,
      IStreamComplete streamComplete) {
    streamNotificationHandlers_.put(key, streamComplete);
  }

  public synchronized static void addStreamContext(String key,
      StreamContext streamContext, StreamStatus streamStatus) {
