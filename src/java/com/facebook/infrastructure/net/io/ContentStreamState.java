...
class ContentStreamState extends StartState {
  ...
  ContentStreamState(TcpReader stream) {
    ...
    InetSocketAddress remoteAddress = (InetSocketAddress) socketChannel
  ...
  private void createFileChannel() throws IOException {
    if (fc_ == null) {
      ...
      FileOutputStream fos = new FileOutputStream(streamContext_
          .getTargetFile(), true);
      fc_ = fos.getChannel();
    }
  }
  ...
  public byte[] read() throws IOException, ReadNotCompleteException {
    ...
    if (streamContext_ != null) {
      try {
        ...
        streamStatus_
            .setAction(StreamContextManager.StreamCompletionAction.STREAM);
  ...
  private void handleStreamCompletion(String remoteHost) throws IOException {
    /*
     * Streaming is complete. If all the data that has to be received inform the
     * sender via the stream completion callback so that the source may perform
     * the requisite cleanup.
     */
    IStreamComplete streamComplete = StreamContextManager
        .getStreamCompletionHandler(remoteHost);
    if (streamComplete != null) {
      streamComplete.onStreamCompletion(remoteHost, streamContext_,
          streamStatus_);
    }
  }
  ...
  public void morphState() throws IOException {
    /*
     * We instantiate an array of size 1 so that we can exit the event loop of
     * the read.
     */
    StartState nextState = stream_
        .getSocketState(TcpReader.TcpReaderState.DONE);
    if (nextState == null) {
