...
public class ProtocolHeaderState extends StartState {
    ...
    if (version <= MessagingService.getVersion()) {
      if (stream_.getProtocolHeader().isStreamingMode_) {
        StartState nextState = stream_
            .getSocketState(TcpReader.TcpReaderState.CONTENT_STREAM);
        if (nextState == null) {
          nextState = new ContentStreamState(stream_);
          stream_.putSocketState(TcpReader.TcpReaderState.CONTENT_STREAM,
              nextState);
        }
        ...
        StartState nextState = stream_
            .getSocketState(TcpReader.TcpReaderState.CONTENT_LENGTH);
        if (nextState == null) {
          nextState = new ContentLengthState(stream_);
          stream_.putSocketState(TcpReader.TcpReaderState.CONTENT_LENGTH,
              nextState);
        }
        stream_.morphState(nextState);
        buffer_.clear();
  ...
  public void setContextData(Object data) {
    throw new UnsupportedOperationException(
        "This method is not supported in the ProtocolHeaderState");
  }
}
