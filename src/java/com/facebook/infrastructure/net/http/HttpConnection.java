 ...
 * @author kranganathan
 */
public class HttpConnection extends SelectionKeyHandler implements
    HttpStartLineParser.Callback, HttpHeaderParser.Callback {
  ...
  /*
   * These are the callbacks into who ever intends to listen on the client
   * socket.
   */
  ...
  enum ParseState {
    IN_NEW, IN_START, IN_HEADERS, IN_BODY
  }
  ...
  /*
   * This abstraction starts reading the data that comes in on a HTTP request.
   * It accumulates the bytes read into a buffer and passes the buffer to the
   * HTTP parser.
   */
  ...
  /*
   * Read called on the Selector thread. This is called when there is some HTTP
  ...
  /*
   * Process the HTTP commands sent from the client. Reads the socket and parses
   * the HTTP request.
   */
      ...
      case IN_HEADERS:
        if (headerParser_.onMoreBytesNew(bb) == false) {
          ...
          if (currentMsgType_ == HttpMessageType.REQUEST) {
            len = currentRequest_
                .getHeader(HttpProtocolConstants.CONTENT_LENGTH);
            ...
            if (currentRequest_.getVersion().equalsIgnoreCase("HTTP/1.1")) {
               ...
               */
              String val = currentRequest_
                  .getHeader(HttpProtocolConstants.CONNECTION);

              if (val != null
                  && val.equalsIgnoreCase(HttpProtocolConstants.CLOSE)) {
                shouldClose_ = true;
              }
            } else if (currentRequest_.getVersion()
                .equalsIgnoreCase("HTTP/1.0")) {
              /* By default no keep-alive */
              shouldClose_ = true;

              /*
               * Scan all of the headers for keep-alive messages
               */
              String val = currentRequest_
                  .getHeader(HttpProtocolConstants.CONNECTION);

              if (val != null
                  && val.equalsIgnoreCase(HttpProtocolConstants.KEEP_ALIVE)) {
                shouldClose_ = false;
              }
            } else {
              /* Assume 0.9 */
            ...
            logger_
                .warn("in HttpConnection::processInput_() Message type is not set");
            return;
          }
      ...
      case IN_BODY:
        ...
        if (contentLength_ > 0) {
          if (bb.remaining() > contentLength_) {
            ...
            bodyBuffers_.add(((ByteBuffer) bb.duplicate().limit(newLimit))
                .slice());
        ...
        if (done) {
          if (currentMsgType_ == HttpMessageType.REQUEST) {
            // currentRequest_.setParseTime(env_.getCurrentTime() -
            // parseStartTime_);
            ...
            logger_
                .info("Holy shit! We are not supposed to be here - ever !!!");
          } else {
  ...
  private void handleRequest(HttpRequest request) {
    HttpConnection.HttpRequestMessage httpRequestMessage = new HttpConnection.HttpRequestMessage(
        request, this);
    Message httpMessage = new Message(null, HttpConnection.httpStage_,
        HttpConnection.httpRequestVerbHandler_,
        new Object[] { httpRequestMessage });
    MessagingService.receive(httpMessage);
  }

  // HttpStartLineParser.Callback interface implementation
  public void onStartLine(String method, String path, String query,
      String version) {
    logger_.debug("Startline method=" + method + " path=" + path + " query="
