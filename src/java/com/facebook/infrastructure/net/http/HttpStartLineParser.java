...
public class HttpStartLineParser {
  ...
  private enum StartLineParseState {
    EATING_WHITESPACE, READING_METHOD, READING_PATH, READING_QUERY, DECODING_FIRST_CHAR, DECODING_SECOND_CHAR, READING_VERSION, CHECKING_EOL, TO_RESET
  }
  ...
  private void finishLine_() {
    if (callback_ != null) {
      callback_.onStartLine(httpMethod_.toString(), httpPath_.toString(),
          httpQuery_.toString(), httpVersion_.toString());
    }
  }
  ...
  public boolean onMoreBytes(InputStream in) throws HttpParsingException,
      IOException {
    int got;
  ...
  public boolean onMoreBytesNew(ByteBuffer buffer) throws HttpParsingException,
      IOException {
    int got;
