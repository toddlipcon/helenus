...
public class MessagingService implements IMessagingService,
  ...
  /*
   * Indicate if we are currently streaming data to another node or receiving
   * streaming data
   */
  ...
  protected MessagingService() {
    ...
    /*
     * Leave callbacks in the cachetable long enough that any related messages
     * will arrive before the callback is evicted from the table. The
     * concurrency level is set at 128 which is the sum of the threads in the
     * pool that adds shit into the table and the pool that retrives the
     * callback from here.
     */
    int maxSize = MessagingConfig.getMessagingThreadCount();
    callbackMap_ = new Cachetable<String, IAsyncCallback>(
        2 * DatabaseDescriptor.getRpcTimeout());
    taskCompletionMap_ = new Cachetable<String, IAsyncResult>(
        2 * DatabaseDescriptor.getRpcTimeout());

    messageDeserializationExecutor_ = new DebuggableThreadPoolExecutor(maxSize,
        maxSize, Integer.MAX_VALUE, TimeUnit.SECONDS,
    ...
    streamExecutor_ = new DebuggableThreadPoolExecutor(1, 1, Integer.MAX_VALUE,
    ...
    registerVerbHandlers(MessagingService.responseVerbHandler_,
        new ResponseVerbHandler());
    /* register stage for response */
    StageManager.registerStage(MessagingService.responseStage_,
  ...
  public byte[] hash(String type, byte data[]) {
    ...
    try {
      ...
      LogUtil.getLogger(MessagingService.class.getName()).debug(
  ...
  public void listen(EndPoint localEp, boolean isHttp) throws IOException {
    ...
    SelectionKey key = SelectorManager.getSelectorManager().register(
        serverChannel, handler, SelectionKey.OP_ACCEPT);
  ...
  public static TcpConnectionManager getConnectionPool(EndPoint from,
      EndPoint to) {
    String key = from + ":" + to;
    ...
    if (cp == null) {
      ...
      try {
        ...
        if (cp == null) {
          cp = new TcpConnectionManager(MessagingConfig
              .getConnectionPoolInitialSize(), MessagingConfig
              .getConnectionPoolGrowthFactor(), MessagingConfig
              .getConnectionPoolMaxSize(), from, to);
  ...
  public static ConnectionStatistics[] getPoolStatistics() {
    ...
    while (it.hasNext()) {
      ...
      ConnectionStatistics cs = new ConnectionStatistics(cp.getLocalEndPoint(),
          cp.getRemoteEndPoint(), cp.getPoolSize(), cp.getConnectionsInUse());
      stats.add(cs);
    }
    return stats.toArray(new ConnectionStatistics[0]);
  }

  public static TcpConnection getConnection(EndPoint from, EndPoint to)
      throws IOException {
  ...
  private void checkForReservedVerb(String type) {
    if (reservedVerbs_.get(type) != null && verbHandlers_.get(type) != null) {
      throw new IllegalArgumentException(type
          + " is a reserved verb handler. Scram!");
    }
  }
  ...
  public void deregisterAllVerbHandlers(EndPoint localEndPoint) {
    ...
    /*
     * endpoint specific verbhandlers can be distinguished because their key's
     * contain the name of the endpoint.
     */
    while (keys.hasNext()) {
  ...
  public void stream(String file, long startPosition, long total,
    ...
    Runnable streamingTask = new FileStreamTask(file, startPosition, total,
        from, to);
    streamExecutor_.execute(streamingTask);
  }

  /*
   * Does the application determine if we are currently streaming data. This
   * would imply either streaming to a receiver, receiving streamed data or
   * both.
   */
  ...
  public static ByteBuffer packIt(byte[] bytes, boolean compress,
      boolean stream, boolean listening) {
    byte[] size = toByteArray(bytes.length);
    /*
     * Setting up the protocol header. This is 4 bytes long represented as an
     * integer. The first 2 bits indicate the serializer type. The 3rd bit
     * indicates if compression is turned on or off. It is turned off by
     * default. The 4th bit indicates if we are in streaming mode. It is turned
     * off by default. The 5th bit is used to indicate that the sender is not
     * listening on any well defined port. This implies the receiver needs to
     * cache the connection using the port on the socket. The following 3 bits
     * are reserved for future use. The next 8 bits indicate a version number.
     * Remaining 15 bits are not used currently.
     */
    int n = 0;
    ...
    ByteBuffer buffer = ByteBuffer.allocate(16 + header.length + size.length
        + bytes.length);
  ...
  public static ByteBuffer constructStreamHeader(boolean compress,
     ...
     * indicates if compression is turned on or off. It is turned off by
     * default. The 4th bit indicates if we are in streaming mode. It is turned
     * off by default. The following 4 bits are reserved for future use. The
     * next 8 bits indicate a version number. Remaining 15 bits are not used
     * currently.
     */
    int n = 0;
    // Setting up the serializer bit
