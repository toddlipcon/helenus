...
public class BootStrapper implements Runnable {
    ...
    try {
      ...
      for (Range splitRange : rangesSplit) {
        ...
        for (Range subRange : subRanges) {
          ...
          oldRangeToEndPointMap
              .put(subRange, new ArrayList<EndPoint>(replicas));
        }
      }
      ...
      Map<Range, List<EndPoint>> newRangeToEndPointMap = StorageService
          .instance().constructRangeToEndPointMap(newRanges);
  ...
  private Range getMyOldRange() {
    Map<EndPoint, BigInteger> oldEndPointToTokenMap = tokenMetadata_
        .cloneEndPointTokenMap();
    Map<BigInteger, EndPoint> oldTokenToEndPointMap = tokenMetadata_
        .cloneTokenEndPointMap();

    oldEndPointToTokenMap.remove(targets_);
    oldTokenToEndPointMap.remove(tokens_);

    BigInteger myToken = oldEndPointToTokenMap.get(StorageService
        .getLocalStorageEndPoint());
    List<BigInteger> allTokens = new ArrayList<BigInteger>(
        oldTokenToEndPointMap.keySet());
