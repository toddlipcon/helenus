...
/**
 * This verb handler handles the BootstrapMetadataMessage that is sent by the
 * leader to the nodes that are responsible for handing off data.
 */
public class BootstrapMetadataVerbHandler implements IVerbHandler {
  private static Logger logger_ = Logger
      .getLogger(BootstrapMetadataVerbHandler.class);
  ...
  public void doVerb(Message message) {
    ...
    try {
      BootstrapMetadataMessage bsMetadataMessage = BootstrapMetadataMessage
          .serializer().deserialize(bufIn);
      ...
      for (BootstrapMetadata bsmd : bsMetadata) {
        ...
        logger_.debug("Time taken to boostrap " + bsmd.target_ + " is "
  ...
  /*
   * This method needs to figure out the files on disk locally for each range
   * and then stream them using the Bootstrap protocol to the target endpoint.
   */
  private void doTransfer(EndPoint target, List<Range> ranges)
      throws IOException {
    if (ranges.size() == 0) {
    ...
    /*
     * (1) First we dump all the memtables to disk. (2) Run a version of
     * compaction which will basically put the keys in the range specified into
     * a directory named as per the endpoint it is destined for inside the
     * bootstrap directory. (3) Handoff the data.
     */
    ...
    for (String tName : tables) {
      ...
      /*
       * Get the counting bloom filter for each endpoint and the list of files
       * that need to be streamed
       */
      List<String> fileList = new ArrayList<String>();
      BloomFilter.CountingBloomFilter cbf = table.forceCompaction(ranges,
          target, fileList);
  ...
  /**
   * Stream the files in the bootstrap directory over to the node being
   * bootstrapped.
   */
  private void doHandoff(BloomFilter.CountingBloomFilter cbf, EndPoint target,
    ...
    for (File file : files) {
      if (file.getName().indexOf("-Data.db") != -1) {
        RequestCountSampler.Cardinality cardinality = new RequestCountSampler.Cardinality(
            cbf, cbf.count());
        streamContexts[i] = new StreamContextManager.StreamContext(file
            .getAbsolutePath(), file.length(), cardinality);
      } else {
        streamContexts[i] = new StreamContextManager.StreamContext(file
