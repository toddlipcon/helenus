...
/**
 * This class performs the exact opposite of the operations of the Bootstrapper
 * class. Given a bunch of nodes that need to move it determines who they need
 * to hand off data in terms of ranges.
 */
public class LeaveJoinProtocolImpl implements Runnable {
  ...
  public void run() {
    try {
      ...
      Map<EndPoint, BigInteger> endpointToTokenMap = tokenMetadata_
          .cloneEndPointTokenMap();

      Set<BigInteger> oldTokens = new HashSet<BigInteger>(tokenToEndPointMap
          .keySet());
      ...
      Map<Range, List<EndPoint>> oldRangeToEndPointMap = StorageService
          .instance().constructRangeToEndPointMap(oldRanges);

      /* Remove the tokens of the nodes leaving the ring */
      Set<BigInteger> tokens = getTokensForLeavingNodes();
      oldTokens.removeAll(tokens);
      Range[] rangesAfterNodesLeave = StorageService.instance().getAllRanges(
          oldTokens);
      /* Get expanded range to initial range mapping */
      Map<Range, List<Range>> expandedRangeToOldRangeMap = getExpandedRangeToOldRangeMapping(
          oldRanges, rangesAfterNodesLeave);
      /* add the new token positions to the old tokens set */
      for (BigInteger token : tokens_)
        oldTokens.add(token);
      Range[] rangesAfterNodesJoin = StorageService.instance().getAllRanges(
          oldTokens);
      /*
       * replace the ranges that were split with the split ranges in the old
       * configuration
       */
      addSplitRangesToOldConfiguration(oldRangeToEndPointMap,
          rangesAfterNodesJoin);
      ...
      /*
       * Remove the old locations from tokenToEndPointMap and add the new
       * locations they are moving to
       */
      ...
      Map<Range, List<EndPoint>> newRangeToEndPointMap = StorageService
          .instance()
          .constructRangeToEndPointMap(newRanges, tokenToEndPointMap);
      /*
       * Remove any expanded ranges and replace them with ranges whose aggregate
       * is the expanded range in the new configuration.
       */
      removeExpandedRangesFromNewConfiguration(newRangeToEndPointMap,
          expandedRangeToOldRangeMap);
      /* Calculate ranges that need to be sent and from whom to where */
      Map<Range, List<BootstrapSourceTarget>> rangesWithSourceTarget = LeaveJoinProtocolHelper
          .getRangeSourceTargetInfo(oldRangeToEndPointMap,
      ...
      /*
       * Send messages to respective folks to stream data over to the new nodes
       * being bootstrapped
       */
  ...
  /**
   * This method figures out the ranges that have been split and replaces them
   * with the split range.
   * 
   * @param oldRangeToEndPointMap
   *          old range mapped to their replicas.
   * @param rangesAfterNodesJoin
   *          ranges after the nodes have joined at their respective position.
   */
  private void addSplitRangesToOldConfiguration(
      Map<Range, List<EndPoint>> oldRangeToEndPointMap,
      Range[] rangesAfterNodesJoin) {
    /*
     * Find the ranges that are split. Maintain a mapping between the range
     * being split and the list of subranges.
     */
    Map<Range, List<Range>> splitRanges = LeaveJoinProtocolHelper
        .getRangeSplitRangeMapping(oldRangeToEndPointMap.keySet().toArray(
            new Range[0]), tokens_);
    /*
     * Mapping of split ranges to the list of endpoints responsible for the
     * range
     */
    ...
    for (Range splitRange : rangesSplit) {
      replicasForSplitRanges.put(splitRange, oldRangeToEndPointMap
          .get(splitRange));
    }
    ...
    /*
     * Add the subranges of the split range to the map with the same replica
     * set.
     */
    for (Range splitRange : rangesSplit) {
  ...
  /**
   * Reset the newRangeToEndPointMap and replace the expanded range with the
   * ranges whose aggregate is the expanded range. This happens only when nodes
   * leave the ring to migrate to a different position.
   * 
   * @param newRangeToEndPointMap
   *          all new ranges mapped to the replicas responsible for those
   *          ranges.
   * @param expandedRangeToOldRangeMap
   *          mapping between the expanded ranges and the ranges whose aggregate
   *          is the expanded range.
   */
  private void removeExpandedRangesFromNewConfiguration(
      Map<Range, List<EndPoint>> newRangeToEndPointMap,
      Map<Range, List<Range>> expandedRangeToOldRangeMap) {
    ...
    for (Range expandedRange : expandedRanges) {
      replicasForExpandedRanges.put(expandedRange, newRangeToEndPointMap
          .get(expandedRange));
      newRangeToEndPointMap.remove(expandedRange);
    }
    /*
     * replace the expanded ranges in the newRangeToEndPointMap with the
     * subRanges
     */
    for (Range expandedRange : expandedRanges) {
  ...
  /**
   * Here we are removing the nodes that need to leave the ring and trying to
   * calculate what the ranges would look like w/o them. For eg if we remove two
   * nodes A and D from the ring and the order of nodes on the ring is A, B, C
   * and D. When B is removed the range of C is the old range of C and the old
   * range of B. We want a mapping from old range of B to new range of B. We
   * have A----B----C----D----E----F----G and we remove b and e then we want a
   * mapping from (a, c] --> (a,b], (b, c] and (d, f] --> (d, e], (d,f].
   * 
   * @param oldRanges
   *          ranges with the previous configuration
   * @param newRanges
   *          ranges with the target endpoints removed.
   * @return map of expanded range to the list whose aggregate is the expanded
   *         range.
   */
  protected static Map<Range, List<Range>> getExpandedRangeToOldRangeMapping(
      Range[] oldRanges, Range[] newRanges) {
    ...
    /*
     * Remove the ranges that are the same. Now we will be left with the
     * expanded ranges in the nRanges list and the smaller ranges in the oRanges
     * list.
     */
    ...
    /*
     * Establish the mapping between expanded ranges to the smaller ranges whose
     * aggregate is the expanded range.
     */
  ...
  public static void main(String[] args) throws Throwable {
    ...
    Runnable runnable = new LeaveJoinProtocolImpl(new EndPoint[] {
        new EndPoint("C", 7000), new EndPoint("D", 7000) }, new BigInteger[] {
        BigInteger.valueOf(22), BigInteger.valueOf(23) });
    runnable.run();
  }
}
