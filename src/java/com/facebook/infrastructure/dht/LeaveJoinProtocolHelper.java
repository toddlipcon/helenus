...
class LeaveJoinProtocolHelper {
  private static Logger logger_ = Logger
      .getLogger(LeaveJoinProtocolHelper.class);

  /**
   * Give a range a-------------b which is being split as a-----x-----y-----b
   * then we want a mapping from (a, b] --> (a, x], (x, y], (y, b]
   */
  protected static Map<Range, List<Range>> getRangeSplitRangeMapping(
      Range[] oldRanges, BigInteger[] allTokens) {
  ...
  protected static Map<Range, List<BootstrapSourceTarget>> getRangeSourceTargetInfo(
    ...
    /*
     * Basically calculate for each range the endpoints handling the range in
     * the old token set and in the new token set. Whoever gets bumped out of
     * the top N will have to hand off that range to the new dude.
     */
    Set<Range> oldRangeSet = oldRangeToEndPointMap.keySet();
    for (Range range : oldRangeSet) {
      logger_
          .debug("Attempting to figure out the dudes who are bumped out for "
              + range + " ...");
      ...
      logger_.debug("Done figuring out the dudes who are bumped out for range "
          + range + " ...");
    }
    for (Range range : oldRangeSet) {
      List<EndPoint> oldEndPoints = oldRangeToEndPointMap.get(range);
      ...
      for (EndPoint oldEndPoint : oldEndPoints) {
        srcTarget.add(new BootstrapSourceTarget(oldEndPoint, newEndPoints
            .get(i++)));
      }
    }
  ...
  /**
   * This method sends messages out to nodes instructing them to stream the
   * specified ranges to specified target nodes.
   */
  protected static void assignWork(
  ...
  protected static void assignWork(
      Map<Range, List<BootstrapSourceTarget>> rangesWithSourceTarget,
      List<EndPoint> filters) throws IOException {
    /*
    ...
    for (Range range : ranges) {
      List<BootstrapSourceTarget> rangeSourceTargets = rangesWithSourceTarget
          .get(range);
      for (BootstrapSourceTarget rangeSourceTarget : rangeSourceTargets) {
        Map<EndPoint, List<Range>> targetRangeMap = rangeInfo
            .get(rangeSourceTarget.source_);
        ...
        List<Range> rangesToGive = targetRangeMap
            .get(rangeSourceTarget.target_);
    ...
    for (EndPoint source : sources) {
      ...
      BootstrapMetadataMessage bsMetadataMessage = new BootstrapMetadataMessage(
          bsmdList.toArray(new BootstrapMetadata[0]));
      /* Send this message to the source to do his shit. */
      Message message = BootstrapMetadataMessage
          .makeBootstrapMetadataMessage(bsMetadataMessage);
