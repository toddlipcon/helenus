...
/**
 * This class is used to load the storage endpoints with the relevant data The
 * data should be both what they are responsible for and what should be
 * replicated on the specific endpoints. Population is done based on a xml file
 * which should adhere to a schema.
 * 
...
public class Loader {
  ...
  /*
   * This method loads all the keys into a special column family called
   * "RecycleBin". This column family is used for temporary processing of data
   * and then can be recycled. The idea is that after the load is complete we
   * have all the keys in the system. Now we force a compaction and examine the
   * single Index file that is generated to determine how the nodes need to
   * relocate to be perfectly load balanced.
   * 
   * param @ rootDirectory - rootDirectory at which the parsing begins. param @
   * table - table that will be populated. param @ cfName - name of the column
   * that will be populated. This is passed in so that we do not unncessary
   * allocate temporary String objects.
   */
  private void preParse(File rootDirectory, String table, String cfName)
      throws Throwable {
  ...
  /*
   * This function checks if the local storage endpoint is reponsible for
   * storing this key .
   */
  ...
  /*
   * This functions parses each file based on delimiters specified in the xml
   * file. It also looks at all the parameters specified in teh xml and based on
   * that populates the internal Row structure.
   */
    ...
    while ((line = bufReader.readLine()) != null) {
      ...
      if (importer_.columnFamily.superColumn != null) {
        ...
        if (importer_.columnFamily.superColumn.tokenize) {
          ...
          TokenStream ts = analyzer.tokenStream("superColumn",
              new StringReader(superColumnName));
          Token token = null;
          token = ts.next();
          while (token != null) {
        ...
        for (String sName : superColumnList) {
          ...
          if (importer_.columnFamily.column != null) {
            for (ColumnType column : importer_.columnFamily.column) {
              String cfColumn = cfName
                  + ":"
                  + (column.name == null ? tokenList.get(column.field)
                      : column.name);
              rm.add(cfColumn, tokenList.get(column.value.field).getBytes(),
                  Integer.parseInt(tokenList.get(column.timestamp.field)));
            }
          }
        ...
        if (importer_.columnFamily.column != null) {
          for (ColumnType column : importer_.columnFamily.column) {
            String cfColumn = importer_.columnFamily.name
                + ":"
                + (column.name == null ? tokenList.get(column.field)
                    : column.name);
            rm.add(cfColumn, tokenList.get(column.value.field).getBytes(),
  ...
  void preLoad(File rootDirectory) throws Throwable {
    ...
    /*
     * This is a hack to let everyone finish. Just sleep for a couple of
     * minutes.
     */
    ...
    /*
     * This is a hack to let everyone relocate and learn about each other. Just
    ...
    /*
     * Do the cleanup necessary. Delete all commit logs and the SSTables and
     * reset the load state in the StorageService.
     */
    SSTable.delete(ssTables.get(0));
    // File commitLogDirectory = new File(
  ...
  void load(String xmlFile) throws Throwable {
    try {
      JAXBContext jc = JAXBContext.newInstance(this.getClass().getPackage()
          .getName());
