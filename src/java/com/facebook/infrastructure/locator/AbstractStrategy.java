...
/**
 * This class contains a helper method that will be used by all abstraction that
 * implement the IReplicaPlacementStrategy interface.
 */
public abstract class AbstractStrategy implements IReplicaPlacementStrategy {
  ...
  /*
   * This method changes the ports of the endpoints from the control port to the
   * storage ports.
   */
  ...
  protected EndPoint getNextAvailableEndPoint(EndPoint startPoint,
    ...
    for (int i = startIndex, count = 1; count < totalNodes; ++count, i = (i + 1)
        % totalNodes) {
  ...
  /*
   * This method returns the hint map. The key is the endpoint on which the data
   * is being placed and the value is the endpoint which is in the top N. Get
   * the map of top N to the live nodes currently.
   */
  public Map<EndPoint, EndPoint> getHintedStorageEndPoints(BigInteger token) {
    ...
    for (int i = 0; i < topN.length; i++) {
      if (FailureDetector.instance().isAlive(topN[i])) {
        ...
        EndPoint endPoint = getNextAvailableEndPoint(topN[i], Arrays
            .asList(topN), liveList);
        if (endPoint != null) {
          ...
          logger_
              .warn("Unable to find a live Endpoint we might be out of live nodes , This is dangerous !!!!");
        }
      }
    }
    return map;
